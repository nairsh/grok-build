...
//! [`crate::trust::TrustStore`] (`~/.atlas/trusted_folders.toml`). The
...
/// this binary — true on a local/dev build (no `ATLAS_VERSION` release stamp).
///
...
/// (`ATLAS_VERSION` unset at compile time) — i.e. a local/dev build.
///
...
fn is_local_build() -> bool {
    // Runtime escape hatch: a pinned ATLAS_TEST_VERSION simulates a release build,
    ...
    option_env!("ATLAS_VERSION").is_none()
}

/// Resolve whether the folder-trust gate is enabled.
///
/// On a local/dev build (no `ATLAS_VERSION` release stamp) the feature is OFF
...
/// env `ATLAS_FOLDER_TRUST` > `[folder_trust] enabled` (user) > managed >
...
fn feature_enabled_for_build(remote: Option<&RemoteSettings>, is_local_build: bool) -> bool {
    ...
    // shipped, release-stamped binaries. Even an explicit ATLAS_FOLDER_TRUST/config
    ...
    BoolFlag::env("ATLAS_FOLDER_TRUST")
...
/// same `~/.atlas/trusted_folders.toml`. Best-effort; a write failure is logged,
/// not fatal.
...
/// `.claude` / `.atlas/agents` / subdir-launch gaps. NOT itself the trust gate.
...
fn collect_repo_config_kinds(cwd: &Path, first_only: bool) -> Vec<&'static str> {
    ...
    // Project `.atlas/config.toml` declaring repo-controlled code-exec: a
    ...
    // Project `.atlas/lsp.json`.
    if cwd.join(".atlas").join("lsp.json").is_file() {
        hit!("lsp");
    }
    ...
    // must be gated — else a hooks-only clone (e.g. `.atlas/hooks/evil.json`) would
    ...
    if hook_root.join(".atlas").join("hooks").is_dir()
    ...
    // that must be gated — else a plugin clone (e.g. `.atlas/plugins/evil/`, even
    ...
    // Project AGENT dirs (`.atlas/agents` / `.claude/agents`): a project agent
...
pub fn prompt_for_trust(key: &Path) -> bool {
    ...
    let _ = writeln!(
        err,
        "This folder contains repo-local config (.mcp.json / .atlas/lsp.json / hooks) \
...
mod tests {
    ...
    fn repo_configs_present_detects_project_agents() {
        // A `.atlas/agents`-only clone must be gated: a project agent definition
    ...
    fn repo_configs_present_detects_project_plugins() {
        ...
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".atlas").join("plugins").join("x")).unwrap();
        assert!(repo_configs_present(tmp.path()));
    }
    ...
    fn repo_configs_present_detects_project_plugins_in_subdir() {
        ...
        std::fs::create_dir_all(subdir.join(".atlas").join("plugins").join("evil")).unwrap();
        assert!(repo_configs_present(&subdir));
    }
    ...
    fn repo_config_kinds_matches_gate_and_reports_all_kinds() {
        ...
        // `.atlas/agents` — even when launched from a SUBDIR (the cwd→git-root walk
    ...
    // ATLAS_HOME-isolation idiom mirrored from this crate's `permission::claude_compat`
    ...
    fn local_build_ignores_remote_rollout() {
        ...
        let _home = EnvVarGuard::set("ATLAS_HOME", home.path());
        let _flag = EnvVarGuard::unset("ATLAS_FOLDER_TRUST");
    ...
    fn release_build_keeps_gate_when_enabled() {
        ...
        // config nor an ambient env flag can override it: empty ATLAS_HOME (no
        // config.toml/managed_config.toml) + ATLAS_FOLDER_TRUST unset. nextest's
    ...
    fn local_build_ignores_explicit_env_optin() {
        ...
        // ATLAS_FOLDER_TRUST=1 does NOT enable the feature (so a self-built grok
        // never prompts). ATLAS_HOME isolated so on-disk config can't influence it.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home = EnvVarGuard::set("ATLAS_HOME", home.path());
        let _flag = EnvVarGuard::set("ATLAS_FOLDER_TRUST", Path::new("1"));
    ...
    fn release_build_defaults_on() {
        ...
        // the feature ON. Empty ATLAS_HOME (no config.toml/managed config) +
        // ATLAS_FOLDER_TRUST unset so only the default applies.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home = EnvVarGuard::set("ATLAS_HOME", home.path());
        let _flag = EnvVarGuard::unset("ATLAS_FOLDER_TRUST");
    ...
    fn is_local_build_honors_test_version_override() {
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        // A pinned ATLAS_TEST_VERSION simulates a release build => not a local build.
        {
            let _sim = EnvVarGuard::set(xai_grok_version::TEST_VERSION_ENV, Path::new("0.0.0-sim"));
            assert!(!is_local_build());
        }
        // With it unset, an unstamped build (no ATLAS_VERSION) is a local build.
        ...
        if option_env!("ATLAS_VERSION").is_none() {
            assert!(is_local_build());
        }
    }
    ...
    fn store_io_is_noop_on_local_build() {
        ...
        // (mirrors `is_local_build_honors_test_version_override`); ATLAS_HOME-isolated
        // and ENV_LOCK-serialized so toggling ATLAS_TEST_VERSION is race-safe.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home = EnvVarGuard::set("ATLAS_HOME", home.path());
        let _unset = EnvVarGuard::unset(xai_grok_version::TEST_VERSION_ENV);
    ...
    fn revoke_folder_trust_store_persists_untrust_for_trusted_folder() {
        ...
        // ATLAS_HOME-isolated so the seed/deny hit a temp store, not the real file.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _env = EnvVarGuard::set("ATLAS_HOME", home.path());
        let _sim = simulate_release_build();
        let tmp = repo_tmp();
        let key = workspace_key(tmp.path());
    ...
    fn revoke_folder_trust_store_writes_no_deny_for_never_trusted_folder() {
        ...
        // ATLAS_HOME-isolated so the grant writes to a temp store.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _env = EnvVarGuard::set("ATLAS_HOME", home.path());
        let _sim = simulate_release_build();
