...
//! `.atlas/config.toml` files by walking from `cwd` up to the git root.
//!
...
/// True when `config_path` is `$ATLAS_HOME/config.toml` (user tier, not project).
...
/// Find all `.atlas/config.toml` files from `cwd` upward to the git repo root.
...
///
/// If no git repo is found, only checks `cwd/.atlas/config.toml`. Excludes the
/// user-global config so `cwd == $HOME` does not treat `~/.atlas/config.toml` as
/// a project overlay.
...
/// `cwd == $HOME` does not treat `~/.atlas/config.toml` as a project overlay.
/// `pub(crate)` — the gate (`repo_configs_present`) reaches it within this crate.
pub(crate) fn find_project_configs_in(chain_dirs: &[PathBuf]) -> Vec<PathBuf> {
    ...
    chain_dirs
        .iter()
        .rev()
        .map(|dir| dir.join(".atlas").join("config.toml"))
...
mod tests {
    ...
    fn find_project_configs_excludes_user_grok_config_file() {
        ...
        std::fs::create_dir_all(project.join(".atlas")).unwrap();
        std::fs::write(project.join(".atlas/config.toml"), "# project\n").unwrap();
