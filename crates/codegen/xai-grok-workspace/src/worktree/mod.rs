...
/// the metadata DB always live under the same `.atlas` tree. That resolver
...
/// Resolves the worktree base directory (`~/.atlas/worktrees/<repo_name>`)
/// for a given source path, correctly handling grok-managed worktrees.
///
/// When `source_path` is already under `~/.atlas/worktrees/<repo>/...`, the
...
/// Walk up from `cwd` (staying within `~/.atlas/worktrees/`) to its registered
/// worktree record.
///
...
/// returns `None` (without DB I/O) for paths outside `~/.atlas/worktrees/`.
...
///
/// When the source path is already inside `~/.atlas/worktrees/<repo>/`, the
...
mod tests {
    ...
    /// Point `ATLAS_HOME` at an isolated tempdir (`resolve_grok_home` re-reads
    ...
    /// and restores `ATLAS_HOME` on drop (before releasing the lock), so the
