...
pub async fn discover_agents_md(root_cwd: &Path) -> Vec<Value> {
    ...
    files
        ...
        .map(|mut file| {
            ...
            if file.file_path.contains("/.atlas/rules/")
...
/// Load the project config from `<root_cwd>/.atlas/config.toml`.
///
...
pub fn load_project_config(root_cwd: &Path) -> Value {
    let config_path = root_cwd.join(".atlas").join("config.toml");
...
mod tests {
    ...
    async fn discover_skills_finds_skill_md() {
        ...
        let skills_dir = tmp.path().join(".atlas").join("skills").join("my-skill");
    ...
    async fn discover_skills_respects_ignore_config() {
        let tmp = tempfile::tempdir().unwrap();
        let skills_dir = tmp.path().join(".atlas").join("skills").join("ignored");
        fs::create_dir_all(&skills_dir).unwrap();
        fs::write(
            skills_dir.join("SKILL.md"),
            "---\nname: ignored\ndescription: Ignored\n---\n",
    ...
    async fn discover_agents_md_strips_rules_frontmatter() {
        let tmp = tempfile::tempdir().unwrap();
        let rules_dir = tmp.path().join(".atlas").join("rules");
        ...
        let rule = files
            ...
            .find(|f| {
                f["file_path"]
                    .as_str()
                    .is_some_and(|p| p.ends_with("/.atlas/rules/xyzzy-discover-agents-md-test.md"))
            })
    ...
    fn discover_plugins_finds_manifest_plugin() {
        let tmp = tempfile::tempdir().unwrap();
        let plugins_dir = tmp.path().join(".atlas").join("plugins").join("test-plugin");
    ...
    fn discover_plugins_json_has_expected_fields() {
        let tmp = tempfile::tempdir().unwrap();
        let plugins_dir = tmp.path().join(".atlas").join("plugins").join("field-test");
        fs::create_dir_all(plugins_dir.join("skills")).unwrap();
        fs::write(
            plugins_dir.join("plugin.json"),
