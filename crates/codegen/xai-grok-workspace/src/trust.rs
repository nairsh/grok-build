...
//! home that resolves to `None` (rather than a cwd-relative `./.atlas`) when
//! neither `$ATLAS_HOME` nor a home directory is set (e.g. a minimal container /
...
//! a `./.atlas/trusted_folders.toml` that self-trusts its own checkout (fail
//! closed).
...
impl TrustStore {
    ...
    /// `./.atlas` — that fallback would let an untrusted cloned repo's `.atlas`
...
/// ROOT (via the `~/.atlas/worktrees.db` registry), so every `grok -w` worktree
...
/// sharing `~/.atlas/`) across the whole read-modify-write so updates merge
...
///
/// The legacy `~/.atlas/trusted-hook-projects` file listed one canonical project
...
mod tests {
    ...
    fn default_path_in_maps_home_and_preserves_no_home() {
        ...
        let home = PathBuf::from("/home/alice/.atlas");
        assert_eq!(
        ...
        // cwd-relative `./.atlas` that grok_home() would invent, which is exactly
        // how a cloned repo's own `<repo>/.atlas/trusted_folders.toml` could
        // masquerade as the user-global store and self-trust the checkout.
    ...
    /// Point `ATLAS_HOME` at an isolated tempdir and register one grok-managed
    ...
    /// dir)`; the [`LockedTestEnv`] holds the lock and restores `ATLAS_HOME` on
    ...
    fn register_grok_worktree(
        ...
        let env = LockedTestEnv::lock().set("ATLAS_HOME", &home);
