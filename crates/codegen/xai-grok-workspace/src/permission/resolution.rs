//! Permission resolution engine: merges native `.atlas/config.toml`,
...
/// Find every `<dir>/.atlas/config.toml` from `cwd` upward to the git repo
/// root (or just `<cwd>/.atlas/config.toml` when there is no git repo).
///
...
fn find_project_grok_configs(cwd: &Path) -> Vec<PathBuf> {
    ...
    if let Some(ref root) = git_root {
        ...
        while let Some(dir) = current {
            let p = dir.join(".atlas").join("config.toml");
            if p.is_file() {
                configs.push(p);
            }
            if dir == *root {
        ...
        let p = cwd.join(".atlas").join("config.toml");
        if p.is_file() {
            configs.push(p);
        }
    }
...
///
///   * `~/.atlas/config.toml` (lowest priority)
///   * Each `.atlas/config.toml` from the git repo root down to `cwd`
...
fn load_config_toml_permissions(cwd: &Path) -> Vec<Sourced<PermissionRule>> {
    ...
    // Gated on user_grok_home() so a project's .atlas/config.toml is never read as
    // global permissions when neither ATLAS_HOME nor a home dir resolves.
...
/// without `~/.atlas`); `path` only names the layer in a non-bool warning.
...
mod tests {
    ...
    // Shared so `ATLAS_HOME`/`HOME` mutations here also serialize against the
    ...
    fn load_claude_env_merges_with_precedence() {
        // ATLAS_HOME-isolate so the claude-import marker reads clean (an imported
        ...
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home_guard = EnvVarGuard::set("ATLAS_HOME", home.path());
    ...
    fn load_claude_env_empty_when_no_settings() {
        // Isolate ATLAS_HOME (claude-import marker) AND HOME (global `~/.claude`)
    ...
    fn load_claude_env_with_project_drops_repo_env_when_untrusted() {
        ...
        // drop it. Isolate ATLAS_HOME so the claude-import marker reads clean (an
    ...
    fn admin_source_trusts_only_root_owned_tiers() {
        ...
        assert!(!is_admin_source(&RequirementSource::Requirements {
            path: "/home/u/.atlas/requirements.toml".into(),
        }));
