...
/// Directory (under `<cwd>/.atlas`) holding every session's checkpoint store.
...
pub(crate) struct CheckpointStore {
    /// Per-session store directory: `<cwd>/.atlas/rewind-checkpoints/<session_id>`.
    dir: PathBuf,
