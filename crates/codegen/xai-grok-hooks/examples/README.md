...
Sample hooks for Grok. Copy to `~/.atlas/hooks/` to enable globally, or to `<project>/.atlas/hooks/` for project-scoped hooks (requires `/hooks-trust`).
...
**Install:**
```sh
mkdir -p ~/.atlas/hooks/bin
cp examples/hooks/safe-shell.json ~/.atlas/hooks/
cp examples/hooks/bin/safe-shell-guard.sh ~/.atlas/hooks/bin/
chmod +x ~/.atlas/hooks/bin/safe-shell-guard.sh
```
...
```sh
mkdir -p ~/.atlas/hooks/bin
cp examples/hooks/no-recursive-grep.json ~/.atlas/hooks/
cp examples/hooks/bin/no-recursive-grep-guard.py ~/.atlas/hooks/bin/
chmod +x ~/.atlas/hooks/bin/no-recursive-grep-guard.py
```
...
Appends session metadata to `~/.atlas/session-audit.log` — event, session ID, cwd, timestamp.

**Install:**
```sh
mkdir -p ~/.atlas/hooks/bin
cp examples/hooks/session-log.json ~/.atlas/hooks/
cp examples/hooks/bin/session-log.sh ~/.atlas/hooks/bin/
chmod +x ~/.atlas/hooks/bin/session-log.sh
```
...
Logs all tool calls to `~/.atlas/tool-activity.log` — tool name, event type, effective tool name, backgrounded status.

**Install:**
```sh
mkdir -p ~/.atlas/hooks/bin
cp examples/hooks/tool-logger.json ~/.atlas/hooks/
cp examples/hooks/bin/tool-logger.sh ~/.atlas/hooks/bin/
chmod +x ~/.atlas/hooks/bin/tool-logger.sh
```
...
Remove the JSON file from `~/.atlas/hooks/`. The hook stops running on the next session.