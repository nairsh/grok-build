...
async fn hook_receives_env_vars() {
    ...
    let cmd = format!(
        r#"echo "EVENT=$ATLAS_HOOK_EVENT" > {f}; echo "NAME=$ATLAS_HOOK_NAME" >> {f}; echo "SESSION=$ATLAS_SESSION_ID" >> {f}; echo '{{"decision":"allow"}}'"#,
...
/// runner-reserved keys (`ATLAS_HOOK_EVENT`, `ATLAS_HOOK_NAME`,
/// `ATLAS_SESSION_ID`, `ATLAS_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`)
...
async fn runner_injected_vars_override_extra_env_at_spawn() {
    ...
    let cmd = format!(
        r#"echo "EVENT=$ATLAS_HOOK_EVENT" > {f}; echo "NAME=$ATLAS_HOOK_NAME" >> {f}; echo "SESSION=$ATLAS_SESSION_ID" >> {f}; echo "ROOT=$ATLAS_WORKSPACE_ROOT" >> {f}; echo "PROJ=$CLAUDE_PROJECT_DIR" >> {f}; echo "USER_KEY=$USER_KEY" >> {f}; echo '{{"decision":"allow"}}'"#,
        f = output_file.display(),
    );
                    ...
                    "hooks": [
                        {
                            ...
                            "env": {
                                "ATLAS_HOOK_EVENT": "spoofed_event",
                                "ATLAS_HOOK_NAME": "spoofed_name",
                                "ATLAS_SESSION_ID": "spoofed_session",
                                "ATLAS_WORKSPACE_ROOT": "/spoofed/root",
    ...
    assert!(
        ...
        "ATLAS_HOOK_EVENT must reflect the real event, got:\n{captured}"
    );
    assert!(
        !captured.contains("EVENT=spoofed_event"),
        "spoofed ATLAS_HOOK_EVENT must NOT leak through, got:\n{captured}"
    );
    assert!(
        captured.contains(&format!("SESSION={real_session}")),
        "ATLAS_SESSION_ID must reflect the real session, got:\n{captured}"
    );
    assert!(
        !captured.contains("SESSION=spoofed_session"),
        "spoofed ATLAS_SESSION_ID must NOT leak through"
    );
    assert!(
        captured.contains(&format!("ROOT={real_workspace}")),
        "ATLAS_WORKSPACE_ROOT must reflect the real workspace root, got:\n{captured}"
    );
    assert!(
        !captured.contains("ROOT=/spoofed/root"),
        "spoofed ATLAS_WORKSPACE_ROOT must NOT leak through"
    );
    assert!(
