...
pub struct RawHandler {
    ...
    /// keys (`ATLAS_HOOK_EVENT`, `ATLAS_HOOK_NAME`, `ATLAS_SESSION_ID`,
    /// `ATLAS_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`) are stripped at load
...
pub struct HookSpec {
    ...
    ///    (`ATLAS_HOOK_EVENT`, `ATLAS_HOOK_NAME`, `ATLAS_SESSION_ID`,
    ///    `ATLAS_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`) are stripped at
    ...
    ///    `ATLAS_PLUGIN_ROOT`, `CLAUDE_PLUGIN_ROOT`, `ATLAS_PLUGIN_DATA`,
    ...
    ///    (`ATLAS_HOOK_EVENT`, `ATLAS_HOOK_NAME`, `ATLAS_SESSION_ID`,
    ///    `ATLAS_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`). These are
...
mod tests {
    ...
    fn source_dir_from_file_path() {
        ...
        let (specs, _) = parse_hook_file(json, Path::new("/home/user/.atlas/hooks/safety.json"));
        assert_eq!(specs[0].source_dir, PathBuf::from("/home/user/.atlas/hooks"));
    }
    ...
    fn parse_hook_file_expands_env_var_in_command_from_process_env() {
        let key = "ATLAS_HOOKS_PARSE_TEST_CMD_PROC_ENV";
    ...
    fn parse_hook_file_expands_env_var_in_url_from_process_env() {
        let key = "ATLAS_HOOKS_PARSE_TEST_URL_PROC_ENV";
    ...
    fn parse_hook_file_preserves_unresolved_env_refs_in_command() {
        let key = "ATLAS_HOOKS_PARSE_TEST_NEVER_SET_AT_LOAD_TIME";
    ...
    fn parse_hook_file_preserves_unresolved_env_refs_in_url() {
        let key = "ATLAS_HOOKS_PARSE_TEST_URL_NEVER_SET_AT_LOAD_TIME";
        with_env_var(key, None, || {
            let json = format!(
                r#"{{
                    "hooks": {{
    ...
    fn parse_hook_file_matcher_is_not_env_expanded() {
        let key = "ATLAS_HOOKS_PARSE_TEST_MATCHER_VAR";
    ...
    fn parse_hook_file_matcher_braced_var_is_not_env_expanded() {
        ...
        let key = "ATLAS_HOOKS_PARSE_TEST_MATCHER_BRACED";
    ...
    /// User attempts to set runner-reserved keys (ATLAS_HOOK_*,
    /// ATLAS_SESSION_ID, ATLAS_WORKSPACE_ROOT, CLAUDE_PROJECT_DIR) via
                        ...
                        "hooks": [
                            {
                                ...
                                "command": "echo hi",
                                "env": {
                                    "ATLAS_HOOK_EVENT": "spoofed",
                                    "ATLAS_HOOK_NAME": "spoofed",
                                    "ATLAS_SESSION_ID": "spoofed",
                                    "ATLAS_WORKSPACE_ROOT": "/etc",
