...
//! - PerSession (`ATLAS_DEBUG_LOG=1`): a routing layer fans each session's
//!   firehose to `~/.atlas/debug/<session_id>.txt` (one file per session), with a
...
//! - SingleFile (explicit path via `ATLAS_LOG_FILE` or `ATLAS_DEBUG_LOG=<path>`):
...
impl DebugSource {
    fn label(self) -> &'static str {
        match self {
            Self::GrokLogFile => "ATLAS_LOG_FILE",
            Self::GrokDebugLog => "ATLAS_DEBUG_LOG",
        }
    }
}
...
// Broad firehose filter for the routing/ATLAS_DEBUG_LOG sources: capture our
...
// ATLAS_DEBUG_LOG single-file source (mirrors `default_file_filter`).
fn firehose_filter() -> EnvFilter {
    EnvFilter::new(firehose_directives())
}

// RUST_LOG-respecting filter for the ATLAS_LOG_FILE source: DEBUG default, honor
// RUST_LOG, silence sampling_log (preserves ATLAS_LOG_FILE back-compat).
...
/// filter by source (ATLAS_LOG_FILE respects RUST_LOG). Open failures warn AFTER
...
pub(crate) enum DebugTarget {
    /// `ATLAS_DEBUG_LOG=1` → route per session into `<dir>` (`~/.atlas/debug`).
    PerSession { dir: PathBuf },
    /// An explicit path → one flat `fmt` file, routing bypassed.
    SingleFile { path: PathBuf, src: DebugSource },
}

/// Resolve the debug target, honoring precedence: explicit ATLAS_LOG_FILE wins
/// (single file, RUST_LOG filter); else ATLAS_DEBUG_LOG — a truthy bool routes
/// per session into `~/.atlas/debug`, an explicit path writes a single file.
///
/// Read via `var_os` (not `var`) so a non-UTF-8 path isn't silently dropped.
pub(crate) fn resolve_debug_target() -> Option<DebugTarget> {
    let grok_log_file = std::env::var_os("ATLAS_LOG_FILE");
    let grok_debug_log = std::env::var_os("ATLAS_DEBUG_LOG");
...
/// `~/.atlas/debug` older than [`LOG_RETENTION`] so the dir doesn't grow
...
mod tests {
        ...
        for v in ["0", "false", "off", "no", "", "  "] {
            assert!(
                resolve_debug_target_inner(None, Some(OsStr::new(v)), Path::new("/debug"))
                    .is_none(),
                "expected None for ATLAS_DEBUG_LOG={v:?}"
            );
        }
    }
    ...
    fn resolve_target_debug_log_enabled_is_per_session_dir() {
        for v in ["1", "true", "on", "yes"] {
            ...
            assert_eq!(
                ...
                "expected PerSession for ATLAS_DEBUG_LOG={v:?}"
            );
        }
    }
    ...
    fn resolve_target_empty_log_file_falls_through_to_debug_log() {
        // Empty / whitespace ATLAS_LOG_FILE is treated as unset (mirrors ATLAS_DEBUG_LOG).
        for blank in ["", "   "] {
            let target = resolve_debug_target_inner(
    ...
    fn resolve_target_non_utf8_debug_log_path_is_single_file() {
        ...
        // A non-UTF-8 ATLAS_DEBUG_LOG value is a path, not a bool keyword, and its
