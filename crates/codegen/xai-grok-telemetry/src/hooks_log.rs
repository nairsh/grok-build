...
//! file logger that writes to `~/.atlas/logs/hooks.log`.
//!
//! ## When to use
//!
...
//!
//! ```bash
//! ATLAS_HOOKS_LOG=1 grok              # enable, write to ~/.atlas/logs/hooks.log
//! ATLAS_HOOKS_LOG=/tmp/h.log grok     # write to custom path
//! ATLAS_HOOKS_LOG=0 grok              # explicitly disable
//! tail -f ~/.atlas/logs/hooks.log     # watch in another terminal
//! ```
...
const ENV_HOOKS_LOG: &str = "ATLAS_HOOKS_LOG";
...
///
/// Writes to `~/.atlas/logs/hooks.log` (or custom path via `ATLAS_HOOKS_LOG`).
/// Filters to hooks (`xai_grok_hooks`) and plugins (`xai_grok_agent::plugins`) targets.
/// Set `ATLAS_HOOKS_LOG=0` to disable, `ATLAS_HOOKS_LOG=/path` to redirect.
