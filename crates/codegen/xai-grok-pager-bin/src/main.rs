...
async fn run_setup_command(json: bool) {
    ...
    if !managed_config::has_principal() {
        ...
        eprintln!();
        if cfg!(unix) {
            eprintln!("  export ATLAS_DEPLOYMENT_KEY=<your-key>");
        } else {
            eprintln!("  $env:ATLAS_DEPLOYMENT_KEY=\"<your-key>\"");
        }
        eprintln!("  grok setup");
        eprintln!();
...
const WORKSPACE_COMMAND_ENV: &str = "ATLAS_WORKSPACE_COMMAND";
...
/// The `ATLAS_WORKSPACE_COMMAND` override, if set (`Some(true)`/`Some(false)`);
...
/// Sets `ATLAS_OPEN_DASHBOARD_AT_STARTUP=1` if the user asked for
...
/// (`[dashboard].enabled = false` / `ATLAS_AGENT_DASHBOARD=0`) is a CLI
    ...
    if !xai_grok_pager::views::dashboard::dashboard_enabled() {
        anyhow::bail!(
            "the Agent Dashboard is disabled. Enable it by removing \
             ...
             unsetting ATLAS_AGENT_DASHBOARD=0."
        );
    }
    args.command = None;
    unsafe { std::env::set_var("ATLAS_OPEN_DASHBOARD_AT_STARTUP", "1") };
    Ok(())
}
...
async fn async_main() -> Result<()> {
    ...
    if let Some(ref mode) = args.compaction_mode {
        unsafe { std::env::set_var("ATLAS_COMPACTION_MODE", mode) };
    }
    if let Some(ref detail) = args.compaction_detail {
        unsafe { std::env::set_var("ATLAS_COMPACTION_DETAIL", detail) };
    }
    if args.chat() {
        unsafe {
            std::env::set_var(xai_grok_shell::agent::chat_modes::ATLAS_CHAT_MODE_ENV, "1");
        }
    }
    ...
    if let Some(ref path) = args.debug_file {
        unsafe {
            std::env::set_var("ATLAS_DEBUG_LOG", path);
            std::env::remove_var("ATLAS_LOG_FILE");
        }
    }
    if args.debug || args.debug_file.is_some() {
        ...
        set_if_unset("ATLAS_DEBUG_LOG", "1");
        set_if_unset("ATLAS_HOOKS_LOG", "1");
    }
...
fn build_update_config() -> UpdateConfig {
    ...
    config.npm_registry = std::env::var(obfstr::obfstr!("ATLAS_NPM_REGISTRY"))
        .ok()
...
fn should_check_for_updates(no_auto_update_flag: bool) -> bool {
    ...
    !std::env::var_os("ATLAS_DISABLE_AUTOUPDATER")
...
/// npm shim hardcodes `~/.atlas`, so a custom `ATLAS_HOME` skips here too.
...
mod tests {
    ...
    fn dashboard_subcommand_flags_startup_without_forcing_leader() {
        ...
        assert_eq!(
            std::env::var("ATLAS_OPEN_DASHBOARD_AT_STARTUP").as_deref(),
            Ok("1"),
    ...
    /// `ATLAS_AGENT_DASHBOARD=0` disables the feature — the subcommand
    ...
    fn dashboard_subcommand_errors_when_disabled() {
        ...
        assert!(
            std::env::var("ATLAS_OPEN_DASHBOARD_AT_STARTUP").is_err(),
    ...
    #[serial_test::serial(ATLAS_WORKSPACE_COMMAND)]
    #[test]
    fn workspace_command_env_override_parsing() {
        unsafe { std::env::remove_var("ATLAS_WORKSPACE_COMMAND") };
        assert_eq!(workspace_command_env_override(), None);
        unsafe { std::env::set_var("ATLAS_WORKSPACE_COMMAND", "1") };
        assert_eq!(workspace_command_env_override(), Some(true));
        unsafe { std::env::set_var("ATLAS_WORKSPACE_COMMAND", "off") };
