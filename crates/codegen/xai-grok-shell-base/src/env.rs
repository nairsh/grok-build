...
//! - `ATLAS_GATEWAY_URL` — when set to a valid URL, `MvpAgent` spawns a
...
pub const ATLAS_GATEWAY_URL_ENV: &str = "ATLAS_GATEWAY_URL";
...
pub const ATLAS_DISABLE_CUSTOM_BRIDGE_ENV: &str = "ATLAS_DISABLE_CUSTOM_BRIDGE";
/// `true` when the custom-method bridge passthrough is force-disabled via
/// [`ATLAS_DISABLE_CUSTOM_BRIDGE_ENV`]. Accepts `1`/`true` (case-insensitive).
pub fn custom_bridge_disabled() -> bool {
    std::env::var(ATLAS_DISABLE_CUSTOM_BRIDGE_ENV)
        .map(|v| {
            let v = v.trim();
...
/// Parse `ATLAS_GATEWAY_URL` into a [`url::Url`].
///
/// This build ignores the variable and stays in local mode (warns if set).
pub fn parse_gateway_url() -> Option<url::Url> {
    let raw = std::env::var(ATLAS_GATEWAY_URL_ENV).ok()?;
    if raw.is_empty() {
        return None;
    }
    tracing::warn!(
        env = ATLAS_GATEWAY_URL_ENV,
        "ATLAS_GATEWAY_URL is set but this build does not support gateway-bridge mode; staying in local mode"
    );
    None
}
#[cfg(test)]
mod tests {
    ...
    fn parse_gateway_url_returns_none_when_unset() {
        let _env = EnvVarGuard::remove(ATLAS_GATEWAY_URL_ENV);
        assert!(parse_gateway_url().is_none());
    }
    #[test]
    fn parse_gateway_url_returns_none_when_empty() {
        let _env = EnvVarGuard::set(ATLAS_GATEWAY_URL_ENV, "");
        assert!(parse_gateway_url().is_none());
    }
    #[test]
    fn parse_gateway_url_hard_off_without_gateway_bridge() {
        let _env = EnvVarGuard::set(ATLAS_GATEWAY_URL_ENV, "wss://gateway.example.com/ws");
        assert!(
    ...
    fn custom_bridge_disabled_defaults_false_when_unset() {
        let _env = EnvVarGuard::remove(ATLAS_DISABLE_CUSTOM_BRIDGE_ENV);
        assert!(!custom_bridge_disabled());
    }
    #[test]
    fn custom_bridge_disabled_true_for_one_and_true() {
        for v in ["1", "true", "TRUE", " true "] {
            let _env = EnvVarGuard::set(ATLAS_DISABLE_CUSTOM_BRIDGE_ENV, v);
            assert!(
