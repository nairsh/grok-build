...
//! persisted to `~/.atlas/tip_cursor.json`.
...
/// Load the cursor from `~/.atlas/tip_cursor.json`. Returns 0 on any error.
...
/// Save the cursor to `~/.atlas/tip_cursor.json`. Silently ignores write errors.
...
/// cursor in `~/.atlas/tip_cursor.json`, so every session sees the next tip
