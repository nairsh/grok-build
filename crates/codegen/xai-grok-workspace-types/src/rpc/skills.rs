...
pub enum SkillScope {
    /// cwd/.atlas/skills
    Local,
    /// repo_root/.atlas/skills
    Repo,
    /// ~/.atlas/skills
    User,
    /// ~/.atlas/server-skills (synced from the skill store)
    Server,
...
mod tests {
    ...
    fn skill_info_deserializes_minimal_payload() {
        let raw = serde_json::json!({
            ...
            "path": "/workspace/.atlas/skills/my-skill/SKILL.md",
            "scope": "local",
        });
    ...
    fn skill_info_deserializes_full_payload() {
        let raw = serde_json::json!({
            ...
            "metadata": {"team": "infra"},
            "path": "/root/.atlas/server-skills/deploy/SKILL.md",
            "scope": "server",
            "config_source": {"type": "user", "path": "/root/.atlas/skills"},
            "plugin_name": "infra-plugin",
            "plugin_version": "1.0.0",
            "plugin_root": "/root/.atlas/plugins/infra-plugin",
            "plugin_data": "/root/.atlas/plugin-data/infra-plugin",
            "allowed_tools": ["bash"],
            "model": "grok-4",
            "effort": "high",
            "user_invocable": true,
