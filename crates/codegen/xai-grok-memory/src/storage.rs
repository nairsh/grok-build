...
//! `~/.atlas/memory/{project-slug}-{hash8}/` to avoid polluting the user's repo.
...
/// `~/.atlas/memory/`. Workspace-scoped files live under a directory
/// named `{project-slug}-{hash8}`, e.g. `~/.atlas/memory/xai-a3f7b2c9/`.
#[derive(Debug, Clone)]
pub struct MemoryStorage {
    /// `~/.atlas/memory/`
    global_dir: PathBuf,
    /// `~/.atlas/memory/{project-slug}-{hash8}/`
...
impl MemoryStorage {
    /// Create a new `MemoryStorage` rooted at `~/.atlas/memory/`.
    ///
    ...
    ///
    /// File path: `~/.atlas/memory/{project}-{hash8}/sessions/YYYY-MM-DD-{slug}-{sid8}.md`
    ///
    /// - `date`: e.g. `"2026-02-23"`
