...
//! `off`. Custom profiles via `~/.atlas/sandbox.toml` or `.atlas/sandbox.toml`.
...
/// Load sandbox config from `~/.atlas/sandbox.toml` and `.atlas/sandbox.toml`.
///
...
pub fn load_sandbox_config(workspace: &Path) -> SandboxConfig {
    ...
    // Global config: ~/.atlas/sandbox.toml
    let global_path = grok_home().join("sandbox.toml");
    if let Some(global) = load_config_file(&global_path) {
        config = global;
    }

    // Project config: <workspace>/.atlas/sandbox.toml (additive only)
    let project_path = workspace.join(".atlas").join("sandbox.toml");
...
pub fn sandbox_profile_conflicts(workspace: &Path) -> Vec<String> {
    ...
    let project =
        load_config_file(&workspace.join(".atlas").join("sandbox.toml")).unwrap_or_default();
...
impl ProfileName {
    ...
    pub fn to_capability_set_with_config(
        ...
        // directories like ~/.atlas/ that may not exist on first run.
                ...
                let profile_config = config.profiles.get(name).ok_or_else(|| {
                    anyhow::anyhow!(
                        "Custom sandbox profile '{name}' not found. \
                         Define it in ~/.atlas/sandbox.toml or .atlas/sandbox.toml:\n\n\
                         [profiles.{name}]\n\
                         extends = \"workspace\"\n\
                         read_only = [\"/data\"]\n"
                    )
