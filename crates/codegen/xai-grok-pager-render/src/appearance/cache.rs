...
/// Default ON when unset. The `ATLAS_PROMPT_SUGGESTIONS` env var overrides
...
/// `ATLAS_SCROLL_MODE` overrides `[ui].scroll_mode` (the `ATLAS_SCROLL_SPEED`
...
pub fn load_scroll_mode() -> ScrollMode {
    SCROLL_MODE_LOADED.with(|loaded| {
        if !loaded.get() {
            let from_env = std::env::var("ATLAS_SCROLL_MODE")
                .ok()
...
/// `ATLAS_INVERT_SCROLL` (`1`/`true`/`0`/`false`) overrides `[ui].invert_scroll`.
pub fn load_invert_scroll() -> bool {
    INVERT_SCROLL_LOADED.with(|loaded| {
        if !loaded.get() {
            let from_env = std::env::var("ATLAS_INVERT_SCROLL")
                .ok()
...
/// its own lines-per-tick. `ATLAS_SCROLL_LINES` overrides `[ui].scroll_lines`.
pub fn load_scroll_lines() -> Option<u8> {
    SCROLL_LINES_LOADED.with(|loaded| {
        if !loaded.get() {
            let from_env = std::env::var("ATLAS_SCROLL_LINES")
                .ok()
