...
/// True when `path` is under user [`grok_home()`] (not project `{cwd}/.atlas`).
...
mod tests {
    ...
    fn display_grok_home_prefix_default_install() {
        if std::env::var("ATLAS_HOME").is_ok() {
            return;
        }
    ...
    fn display_user_grok_path_joins_relative() {
        ...
        assert!(path.contains(".atlas") || path.contains("$ATLAS_HOME"));
    }
    ...
    fn abbreviate_path_uses_home_when_under_default_grok() {
        if let Ok(home) = std::env::var("HOME") {
            ...
            let full = format!("{home}/.atlas/memory/MEMORY.md");
