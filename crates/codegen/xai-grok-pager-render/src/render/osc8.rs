...
/// One path segment without spaces (`main.rs`, `.atlas`, `@scope`). Leading `.`
...
/// Relative file path (`images/1.png`, `.atlas/x.txt`) — one or more `/`-joined
...
/// media lives at `~/.atlas/sessions/%2F…/images/1.jpg`, which wraps in
...
mod tests {
    ...
    fn scan_detects_grok_session_media_path() {
        // Dot-directory (`.atlas`), percent-encoded session segment, and a
        // trailing sentence period — the shape of `image_gen` output prose.
        let line = make_line("Saved to /Users/alice/.atlas/sessions/%2Fabc/00000000/images/1.jpg.");
        let mut overlay = LinkOverlay::new();
        scan_unjoined(std::iter::once((0, &line)), 0, &[], &mut overlay);
        ...
        assert_eq!(
            ...
            "file:///Users/alice/.atlas/sessions/%252Fabc/00000000/images/1.jpg",
        );
    }
    ...
    fn scan_detects_media_path_soft_wrapped_across_rows() {
        ...
        let row0 =
            make_line("Image generated and saved to /Users/alice/.atlas/sessions/%2FUsers%2Fali");
        ...
        let expected_url = "file:///Users/alice/.atlas/sessions/%252FUsers%252Fali\
        ...
        assert_eq!(
            ...
            2 + UnicodeWidthStr::width(
                "Image generated and saved to /Users/alice/.atlas/sessions/%2FUsers%2Fali"
            ) as u16
        );
    ...
    fn scan_wrapped_path_trailing_sentence_period_excluded() {
        ...
        let row0 = make_line("Saved to /Users/me/.atlas/sessions/%2Fabc/019f3a86/ima");
        ...
        for link in overlay.links() {
            assert_eq!(
                &*resolve_link_target(&link.target)
                    .and_then(|resolved| resolved.osc8_url)
                    .expect("url"),
                "file:///Users/me/.atlas/sessions/%252Fabc/019f3a86/images/1.jpg"
            );
        }
