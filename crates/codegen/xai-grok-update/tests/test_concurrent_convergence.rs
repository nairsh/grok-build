...
/// Assert the active `~/.atlas/bin/grok` resolves to the expected versioned
...
// installers that actually maintain the managed `~/.atlas/bin/grok` symlink
...
async fn disk_probe_rejects_dangling_symlink() {
    ...
    // ~/.atlas/downloads cleanup), the probe must report None — otherwise
