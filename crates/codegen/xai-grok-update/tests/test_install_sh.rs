...
fn seed_previous_good(home: &Path, platform: &str) -> PathBuf {
    let downloads = home.join(".atlas").join("downloads");
    let bin = home.join(".atlas").join("bin");
...
fn assert_active_grok_runs(home: &Path) {
    let link = home.join(".atlas").join("bin").join("grok");
...
fn run_installer(install_sh: &Path, home: &Path, fakebin: &Path, mode: &str, shell: &str) -> bool {
    ...
    let status = Command::new("/bin/bash")
        ...
        .env("PATH", path_env)
        .env("SHELL", shell)
        .env("ATLAS_BIN_DIR", home.join(".atlas").join("bin"))
        .env("ATLAS_CHANNEL", "stable")
        .env("FAKE_MODE", mode)
        .status()
