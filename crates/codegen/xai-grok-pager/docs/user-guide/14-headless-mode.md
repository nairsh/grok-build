...
| `ATLAS_LOG_FILE`                | Path to a log file (used verbatim as the path; works in headless and TUI, honors `RUST_LOG`) |
...
Grok stores data in `~/.atlas` (override with `ATLAS_HOME`; see [Environment Variables for Headless](#environment-variables-for-headless)):
...
```bash
export XAI_API_KEY="xai-..."
export ATLAS_DISABLE_AUTOUPDATER=1
...
| `ATLAS_DISABLE_AUTOUPDATER=1`    | Process   |
| Non-TTY stderr (auto-detected)  | Automatic |
| `[cli] auto_update = false`     | Persistent|

`ATLAS_DISABLE_AUTOUPDATER` set to a falsy value (`0`, `false`, `off`, `no`, or empty, any
case) counts as not set. The agent SDKs
inject `ATLAS_DISABLE_AUTOUPDATER=1` for the non-leader agents they spawn (a falsy value in
the SDK's isolation env keeps updates on), and the stdio agent skips its background update
unless it runs from the managed install (`$ATLAS_HOME/bin/grok`).
