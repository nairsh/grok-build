...
Sessions are identified by a unique session ID (a UUIDv7 when Grok generates it; a client may supply its own ID with `-s`) and stored on disk under `~/.atlas/sessions/`. Set `ATLAS_HOME` to override the base directory; when it is unset, Grok uses `~/.atlas`.
