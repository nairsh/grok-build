...
- **Ctrl+\\** — same as the slash command, two keystrokes. Configurable
  in `~/.atlas/config.toml` under `[keybindings]` like every other shortcut.
...
Set `ATLAS_AGENT_DASHBOARD=0` to force-disable the feature for a single
