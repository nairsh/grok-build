...
> `ATLAS_MCP_STARTUP_TIMEOUT_SECS` (seconds). A per-server `startup_timeout_sec`
...
>
> - env `ATLAS_MAX_MCP_OUTPUT_BYTES` or `MAX_MCP_OUTPUT_BYTES` (bytes; Grok-native
...
Grok walks from the current directory up to the git repo root, loading `.atlas/config.toml` at each level:
...
Project-scoped files contribute `[mcp_servers]`, `[plugins]`, and `[permission]` entries. Grok reads most other config sections only from `~/.atlas/config.toml`.
...
|--------|--------|----------|-------------|
| `config.toml` | Native Grok config | `~/.atlas/config.toml`, `.atlas/config.toml` | Always on |
...
You must authenticate OAuth-based MCP servers before you can use them. Grok stores the resulting tokens under `~/.atlas/mcp_credentials.json`. After you edit `config.toml`, press `r` in the `/mcps` modal to refresh the server list.
...
For stdio servers, Grok captures the process's standard error to `~/.atlas/logs/mcp/<server>.stderr.log`, truncated on each launch. Check this file when a server starts but fails to handshake:
