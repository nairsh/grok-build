...
Grok uses the API key as a fallback when no session token is active. If you have already signed in interactively, the stored session token takes precedence. To fall back to the API key, run `grok logout` or delete `~/.atlas/auth.json`.
...
export ATLAS_AUTH_PROVIDER_LABEL="Acme Corp"
export ATLAS_AUTH_TOKEN_TTL=3600
```

### Token Refresh

When Grok needs to refresh an expired token, it re-runs your binary with `ATLAS_AUTH_EXPIRED=1` set in the environment. Each run fully replaces the stored credential, so emit the same JSON fields (such as `issuer`) on every invocation, including refreshes. Your binary can use this to take a faster silent-refresh path:
...
|----------|-------------|
| `ATLAS_AUTH_PROVIDER_COMMAND` | Path to your auth binary |
| `ATLAS_AUTH_PROVIDER_LABEL` | Display name on the TUI login screen (e.g., "Acme Corp") |
| `ATLAS_AUTH_TOKEN_TTL` | Token lifetime in seconds (for bare-string tokens without `expires_in`) |
| `ATLAS_AUTH_EXPIRED` | Set to `1` by Grok when re-running the binary for token refresh |
| `ATLAS_AUTH_EARLY_INVALIDATION_SECS` | Seconds before expiry to proactively refresh (default: 300) |
...
export ATLAS_AUTH_EARLY_INVALIDATION_SECS=0
```
...
Grok picks up changes to `~/.atlas/auth.json` automatically. If you update credentials externally (for example, with a script that writes new tokens), Grok uses the new credentials on the next API call without a restart.
...
2. **Enterprise OIDC** -- when OIDC is configured, through `[grok_com_config.oidc]` in `config.toml` or the `ATLAS_OIDC_ISSUER` and `ATLAS_OIDC_CLIENT_ID` environment variables
...
In the TUI, set `ATLAS_LOG_FILE` to an absolute path to write logs to that file:

```bash
ATLAS_LOG_FILE=/tmp/grok.log RUST_LOG=debug grok
tail -f /tmp/grok.log
```

`ATLAS_LOG_FILE` is treated as a literal file path. A relative value such as `1` writes a file named `1` in the current directory.
