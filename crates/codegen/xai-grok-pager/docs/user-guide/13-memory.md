...
3. `ATLAS_MEMORY` env var: `1`/`true` enables, `0`/`false` disables
...
Ask Grok to remember something, and it appends the note to a `MEMORY.md` file -- the workspace file for project-specific items, or the global `~/.atlas/memory/MEMORY.md` for cross-project preferences:
...
By default, Grok watches `~/.atlas/memory/` for external file changes. If you edit memory files directly (e.g., in your editor), the changes are picked up automatically on the next memory search:
...
2. Check the flag: `grok --experimental-memory` or `ATLAS_MEMORY=1`.
3. Check for `--no-memory` or `ATLAS_MEMORY=0` overriding your config.
