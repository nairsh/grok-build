...
```bash
export ATLAS_EXTERNAL_OTEL=1                  # master switch
...
`ATLAS_EXTERNAL_OTEL=1` alone enables **nothing** — you must also select at
...
|---|---|---|
| `ATLAS_EXTERNAL_OTEL` | `0` | Master switch. Distinct from `ATLAS_TELEMETRY_ENABLED`, which controls SpaceXAI-internal product analytics — the two govern opposite-pointing data flows. |
...
> `ATLAS_EXTERNAL_OTEL` is set, product analytics ignores those vars, and the
...
standard OTEL names** (`ATLAS_EXTERNAL_OTEL`, `OTEL_*`) for ecosystem
