...
pub fn derive_source_label(source_dir: &str) -> (String, bool) {
    ...
    // Plugin / installed-plugin dirs, under the user grok home (ATLAS_HOME-aware)
    // or a project-scoped `{cwd}/.atlas/<subdir>/`. Returns the first path
    ...
    // Project hooks
    if source_dir.ends_with("/.atlas/hooks") || source_dir.contains("/.atlas/hooks/") {
...
mod tests {
    ...
    fn derive_source_label_detects_project_scoped_plugins() {
        // Regression: project-scoped `{cwd}/.atlas/plugins/<name>/` must label as
        // a (non-removable) plugin, not a removable "Custom" source. The user
        // grok-home branch is ATLAS_HOME-aware; this covers the project fallback.
        let (label, is_custom) = derive_source_label("/repo/work/.atlas/plugins/my-plugin/hooks");
        assert_eq!(label, "Plugin: my-plugin");
        assert!(!is_custom);

        let (label, is_custom) =
            derive_source_label("/repo/work/.atlas/installed-plugins/vendor-abc123/skills");
    ...
    fn local_plugins_source() -> xai_hooks_plugins_types::MarketplaceScanResult {
        xai_hooks_plugins_types::MarketplaceScanResult {
            ...
            source_url_or_path: "/home/user/.atlas/marketplace/local".into(),
            plugins: vec![
                TestPlugin {
                    name: "my-linter",
                    version: Some("0.1.0"),
