...
/// User-level vs project-level config files (`~/.atlas` vs `{cwd}/.atlas`).
...
pub struct AgentsModalState {
    ...
    bundle: BundleState,
    /// Resolved startup agent name (same chain as shell: `[agent]`, `ATLAS_AGENT`,
...
/// Union bundled personas with local `~/.atlas/personas` and `{cwd}/.atlas/personas`.
///
...
pub fn merge_persona_lists(bundle: &BundleState, cwd: &Path) -> Vec<PersonaDetail> {
    ...
    let dirs = [
        (ConfigFileScope::Project, cwd.join(".atlas").join("personas")),
...
/// True when `path` is a deletable local persona file (user or project `.atlas/personas`).
pub fn persona_path_is_deletable(path: &Path) -> bool {
    config_path_is_user_or_project(path, "personas")
}
/// Shared guard: canonical path under `~/.atlas/{subdir}` or `{cwd}/.atlas/{subdir}`, not bundled.
fn config_path_is_user_or_project(path: &Path, subdir: &str) -> bool {
    ...
    let project_suffix = std::path::Path::new(".atlas").join(subdir);
...
mod tests {
    ...
    fn merge_persona_lists_appends_local_only() {
        ...
        let personas_dir = dir.path().join(".atlas").join("personas");
    ...
    fn persona_is_deletable_local_vs_bundled() {
        let dir = tempfile::tempdir().expect("tempdir");
        let local = dir.path().join(".atlas").join("personas").join("p.toml");
        ...
        let bundled_detail = PersonaDetail {
            ...
            has_inputs: false,
            has_outputs: false,
            source_path: Some("/home/user/.atlas/bundled/personas/b.toml".into()),
            scope_label: None,
        };
    ...
    fn delete_persona_file_allows_project_persona() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = dir.path().join(".atlas").join("personas").join("gone.toml");
