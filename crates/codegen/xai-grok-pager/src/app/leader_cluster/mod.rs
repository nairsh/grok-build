...
//!
//! Env sandboxing follows this crate's `serial(ATLAS_HOME)` idiom; note
...
//! `ATLAS_LEADER_SOCKET`, `ATLAS_HOME`) for a real agent's whole lifetime, and
...
struct PagerLeaderCluster {
    ...
    /// agent on the same ATLAS_HOME (two agents on one updates.jsonl is the
...
impl PagerLeaderCluster {
    /// Stand up the cluster. Callers MUST be `#[serial_test::serial(ATLAS_HOME)]`
    ...
    async fn start() -> Self {
        ...
        let env = vec![
            crate::test_util::EnvVarGuard::set("ATLAS_HOME", grok_home.path()),
            crate::test_util::EnvVarGuard::set("ATLAS_CLI_CHAT_PROXY_BASE_URL", server.url()),
            crate::test_util::EnvVarGuard::set("ATLAS_XAI_API_BASE_URL", server.url()),
            crate::test_util::EnvVarGuard::set("XAI_API_KEY", "test-key-for-ci"),
            crate::test_util::EnvVarGuard::set("ATLAS_TELEMETRY_ENABLED", "false"),
            crate::test_util::EnvVarGuard::set("ATLAS_FEEDBACK_ENABLED", "false"),
            crate::test_util::EnvVarGuard::set("ATLAS_TRACE_UPLOAD", "false"),
    ...
    async fn kill_leader(&mut self) {
        ...
        // be running against the same ATLAS_HOME when the next generation's
    ...
    /// `LeaderReconnector` (socket pinned via `ATLAS_LEADER_SOCKET`, flock held
