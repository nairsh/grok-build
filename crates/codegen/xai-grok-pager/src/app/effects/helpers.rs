...
pub(crate) async fn persist_setting(
    ...
    match key {
        ...
        "task_model" => {
            let SettingValue::String(s) = value else {
                return Err(kind_mismatch("task_model", "String", &value));
            };
            xai_grok_shell::util::config::set_task_model(s)
                .await
                .map_err(|e| e.to_string())
        }
        "explore_model" => {
            let SettingValue::String(s) = value else {
                return Err(kind_mismatch("explore_model", "String", &value));
            };
            xai_grok_shell::util::config::set_explore_model(s)
                .await
                .map_err(|e| e.to_string())
        }
