...
//! `ATLAS_OSC52_SINK` / `LC_GROK_OSC52_SINK`). Do not generalize this pattern
...
/// Successful host image frame: `ATLAS_WRAP_IMG\n<mime>\n<base64>`.
pub const MAGIC_IMG: &str = "ATLAS_WRAP_IMG";

/// Host has no image (`ATLAS_WRAP_NONE`; not a prefix of [`MAGIC_IMG`]).
pub const MAGIC_NONE: &str = "ATLAS_WRAP_NONE";
...
mod tests {
    ...
    fn garbage_is_not_wrap_paste() {
        ...
        assert_eq!(try_decode_wrap_host_image_paste("ATLAS_WRAP_IM"), None);
    }

    #[test]
    fn malformed_img_frame_consumed_not_text() {
        assert_eq!(
            try_decode_wrap_host_image_paste("ATLAS_WRAP_IMG"),
            Some(WrapImagePaste::NoImage)
        );
        assert_eq!(
            try_decode_wrap_host_image_paste("ATLAS_WRAP_IMG\nbad"),
    ...
    fn oversized_b64_rejected_before_decode() {
        ...
        let payload = format!("ATLAS_WRAP_IMG\nimage/png\n{huge}");
        assert_eq!(
