...
pub(crate) fn contextual_hints_env(content: &ContentController) -> Vec<(String, String)> {
    ...
    env.push(("ATLAS_CONTEXTUAL_HINTS".into(), "1".into()));
    env
}
...
/// would never fetch `/v1/settings`. Spawns WITHOUT `ATLAS_ANNOUNCEMENTS_OVERRIDE`
...
pub(crate) fn spawn_polling_session_with_env(
    ...
    env_refs.push(("ATLAS_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS", "1"));
...
/// (`ATLAS_TEST_VERSION`) and an explicit `ATLAS_FOLDER_TRUST` — `1` when `feature_on`,
/// else `0` (an explicit opt-out that overrides the now-on default). HOME/ATLAS_HOME
...
pub(crate) fn trust_env(content: &ContentController, feature_on: bool) -> Vec<(String, String)> {
    ...
    // auto-trust.
    env.push(("ATLAS_TEST_VERSION".into(), "0.0.0-sim".into()));
    // Set ATLAS_FOLDER_TRUST explicitly: the default is on, so `0` is the opt-out
    // that exercises the feature-off path rather than relying on an absent var.
    let folder_trust = if feature_on { "1" } else { "0" };
    env.push(("ATLAS_FOLDER_TRUST".into(), folder_trust.into()));
    env
}
...
pub(crate) fn seed_mcp_server_config(content: &ContentController) {
    ...
    std::fs::create_dir_all(&grok_home).expect("create fake ATLAS_HOME");
    let config = format!(
...
/// Seed `~/.atlas/config.toml` with a `[ui]` section body (e.g.
/// `"vim_mode = true"`). Same `{ATLAS_HOME|HOME}/.atlas/config.toml` location
...
pub(crate) fn seed_mouse_reporting_toggle_config(content: &ContentController, enabled: bool) {
    let grok_home = content.home().join(".atlas");
    std::fs::create_dir_all(&grok_home).expect("create .atlas");
    // Minimal opt-in only — matches load_config's `{ATLAS_HOME|HOME}/.atlas/config.toml`.
...
pub(crate) fn mouse_toggle_env(content: &ContentController) -> Vec<(String, String)> {
    let mut env = content.env_for_pager();
    env.push(("ATLAS_MOUSE_REPORTING_TOGGLE".into(), "true".into()));
    env
}
...
pub(crate) const ESC_DOUBLE_PRESS_ENV: &str = "ATLAS_ESC_DOUBLE_PRESS_MS";
...
/// `ATLAS_HOME`, returning the exit code (`None` if it never exited within
...
pub(crate) fn run_wrap(wrap_args: &[&str], extra_env: &[(&str, &str)]) -> (Option<u32>, String) {
    ...
    let mut env: Vec<(&str, &str)> = vec![("ATLAS_HOME", &home_str), ("NO_COLOR", "1")];
...
/// Dump an asciinema cast of `harness` into `$ATLAS_PTY_CAST_DIR/<file_name>`
...
pub(crate) fn write_cast_if_requested(harness: &PtyHarness, file_name: &str) {
    let Ok(dir) = std::env::var("ATLAS_PTY_CAST_DIR") else {
        return;
    };
    if dir.is_empty() {
        return;
