...
in `.atlas/agents/` (project-level) or `~/.atlas/agents/` (user-level).
...
let def = AgentDefinition::from_file(".atlas/agents/code-reviewer.md")?;
...
// Find all .md files in .atlas/agents/ directories
...
1. **Project-level** (highest priority): `.atlas/agents/*.md` — walk
   from `cwd` up to the git repository root. Files found closer to
   `cwd` take priority.
2. **User-level**: `~/.atlas/agents/*.md`
...
example, a project `.atlas/agents/code-reviewer.md` shadows a
