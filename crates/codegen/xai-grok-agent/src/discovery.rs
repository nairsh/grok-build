...
//!
//! Searches `.atlas/agents/` and `.claude/agents/` from cwd to repo root,
//! then `~/.atlas/agents/`, then `~/.claude/agents/`. Name-based dedup keeps
//! highest priority.
...
/// Project-level agent directories to scan (`.atlas/agents/` + `.claude/agents/` compat).
const PROJECT_AGENT_SUBDIRS: &[&str] = &[".atlas/agents", ".claude/agents"];

/// Existing project-level agent dirs (`.atlas/agents` / `.claude/agents`), walked
...
/// Existing project agent dirs (`.atlas/agents` / `.claude/agents`) under each
...
fn merge_subagents(
    ...
    //
    // A user-level ~/.atlas/agents/explore.md does NOT shadow built-in explore
...
/// 1. `.atlas/agents/` walking from `cwd` up to repo root
/// 2. `~/.atlas/agents/` (user-level)
/// 3. `~/.claude/agents/` (compat user-level)
/// 4. `~/.atlas/bundled/agents/` (bundled, lowest priority)
///
...
/// compat agents, then bundled. `.atlas` dirs resolve from `grok_home`
/// (ATLAS_HOME-aware) plus the legacy literal `~/.atlas` when ATLAS_HOME points
...
pub(crate) fn user_agent_dirs(
    ...
    // (i.e. ATLAS_HOME points elsewhere) so agents left in the old location are
...
///
/// Project-level `.atlas/agents/` has highest priority, then falls back
...
/// Load project agent definitions from every `.atlas/agents` / `.claude/agents`
...
mod tests {
    ...
    fn user_agent_dirs_includes_legacy_grok_when_grok_home_differs() {
        ...
        assert!(paths.contains(&home.join(".atlas").join("agents")));
        assert!(paths.contains(&home.join(".claude").join("agents")));
        assert!(paths.contains(&grok.join("bundled").join("agents")));
        assert!(paths.contains(&home.join(".atlas").join("bundled").join("agents")));
    }
    ...
    fn test_discover_dedup_by_name() {
        ...
        let agents_dir_1 = tmp.path().join(".atlas").join("agents");
        let agents_dir_2 = inner_dir.join(".atlas").join("agents");
    ...
    fn test_discover_includes_bundled_agents_at_lowest_priority() {
        ...
        let bundled_dir = home.join(".atlas").join("bundled").join("agents");
        ...
        let defs = discover_with_home(&cwd, Some(&home), Some(&home.join(".atlas")));
        assert_eq!(defs.len(), 1);
    ...
    fn test_by_name_in_cwd_uses_bundled_when_no_higher_priority_match_exists() {
        ...
        let def =
            by_name_in_cwd_with_home("bundled-only", &cwd, Some(&home), Some(&home.join(".atlas")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_user_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let user_dir = home.join(".atlas").join("agents");
        let bundled_dir = home.join(".atlas").join("bundled").join("agents");
        fs::create_dir_all(&cwd).unwrap();
        ...
        let def =
            by_name_in_cwd_with_home("reviewer", &cwd, Some(&home), Some(&home.join(".atlas")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_builtin_beats_bundled_for_builtin_names() {
        ...
        let def = by_name_in_cwd_with_home("explore", &cwd, Some(&home), Some(&home.join(".atlas")))
            .unwrap();
    ...
    fn test_by_name_in_cwd_project_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let project_dir = cwd.join(".atlas").join("agents");
        let bundled_dir = home.join(".atlas").join("bundled").join("agents");
    ...
    fn test_by_name_in_cwd_falls_back_to_builtin() {
        let tmp = tempfile::tempdir().unwrap();
        // No .atlas/agents/ directory — should fall back to built-in
