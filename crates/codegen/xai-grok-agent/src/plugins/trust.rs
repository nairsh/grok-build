...
//!
//! Plugins from project directories (`.atlas/plugins/`, `.claude/plugins/`)
...
impl TrustStore {
    ...
    pub fn load() -> Self {
        // Gate on user_grok_home() so a project's `.atlas/trusted-plugins` is never
        // read as the user trust store when neither ATLAS_HOME nor a home dir resolves.
