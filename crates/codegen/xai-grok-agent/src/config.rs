//! Agent definition types — parsed from `.atlas/agents/*.md` files.
...
/// Portable agent identity — parsed from .atlas/agents/*.md.
...
pub enum AgentScope {
    /// .atlas/agents/ (project-level, highest priority)
    Project,
...
pub enum MemoryScope {
    ...
    User,
    /// `<project>/.atlas/agent-memory/<name>/`
    Project,
    /// `<project>/.atlas/agent-memory-local/<name>/`
    Local,
}
impl MemoryScope {
    ...
    pub fn resolve_dir(self, agent_name: &str, project_cwd: &std::path::Path) -> ResolvedMemoryDir {
        match self {
            ...
            Self::Project => ResolvedMemoryDir {
                path: project_cwd.join(".atlas/agent-memory").join(agent_name),
                is_project_scoped: true,
            },
            Self::Local => ResolvedMemoryDir {
                path: project_cwd
                    .join(".atlas/agent-memory-local")
                    .join(agent_name),
                is_project_scoped: true,
            },
        }
...
impl AgentDefinition {
    ...
    fn scope_from_path(path: &Path) -> AgentScope {
        ...
        if path_str.contains(".atlas/agents/") || path_str.contains(".atlas\\agents\\") {
            return AgentScope::Project;
        }
        if path_str.contains(".atlas/bundled/agents/")
            || path_str.contains(".atlas\\bundled\\agents\\")
        {
...
"#;
    ...
    fn memory_scope_resolve_dir() {
        ...
        assert_eq!(
            proj.path,
            std::path::PathBuf::from("/project/.atlas/agent-memory/a")
        );
        assert!(proj.is_project_scoped);
        let local = MemoryScope::Local.resolve_dir("a", cwd);
        assert_eq!(
            local.path,
            std::path::PathBuf::from("/project/.atlas/agent-memory-local/a")
        );
