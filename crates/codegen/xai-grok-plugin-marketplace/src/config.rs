//! Parse marketplace sources from `~/.atlas/config.toml`.
//!
//! Expected format:
//! ```toml
//! [[marketplace.sources]]
...
/// and `known_marketplaces.json` files under `~/.atlas/` and `~/.claude/`.
...
/// instead of `~/.atlas`/`~/.claude`. Each root is checked for
