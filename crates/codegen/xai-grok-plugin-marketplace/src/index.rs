...
//!
//! Catalog file lookup, in order: `.atlas-plugin/marketplace.json` (preferred),
//! `.atlas-plugin/plugin.json`, `.claude-plugin/marketplace.json`,
...
///
/// Checks (in order):
/// 1. `.atlas-plugin/marketplace.json` (preferred xAI convention)
/// 2. `.atlas-plugin/plugin.json`
...
pub fn load_index(marketplace_root: &Path) -> Result<Option<MarketplaceIndex>, String> {
    let grok_dir = marketplace_root.join(".atlas-plugin");
...
mod tests {
    ...
    fn load_index_valid_grok_dir() {
        let dir = tempfile::tempdir().unwrap();
        let grok_dir = dir.path().join(".atlas-plugin");
    ...
    fn load_index_grok_dir_takes_precedence_over_claude_dir() {
        let dir = tempfile::tempdir().unwrap();
        for (sub, name) in [(".atlas-plugin", "grok"), (".claude-plugin", "claude")] {
