...
/// Read hidden announcement ids from `~/.atlas/announcements.json`.
...
/// Write hidden announcement ids to `~/.atlas/announcements.json`.
...
///
/// Precedence: `ATLAS_ANNOUNCEMENTS_OVERRIDE` env var (JSON override) → remote announcements.
...
pub fn resolve_startup(
    ...
    if let Ok(raw) = std::env::var("ATLAS_ANNOUNCEMENTS_OVERRIDE") {
        match serde_json::from_str::<Vec<RemoteAnnouncement>>(&raw) {
            Ok(list) => return Some(list),
            Err(e) => {
                tracing::warn!(error = %e, "invalid ATLAS_ANNOUNCEMENTS_OVERRIDE JSON; ignoring override");
            }
        }
    }
...
mod tests {
    ...
    fn resolve_startup_env_override() {
        ...
        unsafe {
            std::env::set_var("ATLAS_ANNOUNCEMENTS_OVERRIDE", r#"[{"id":"test"}]"#);
        }
        ...
        // SAFETY: test-only
        unsafe {
            std::env::remove_var("ATLAS_ANNOUNCEMENTS_OVERRIDE");
        }
    }
