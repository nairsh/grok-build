...
1. If the question is about **current config** (what MCP servers, models, or settings are active),
   read `~/.atlas/config.toml`. MCP servers are under `[mcp_servers.*]` sections.
...
2. If the question is about **how to do something** (setup, adding MCP servers, creating skills,
   ...
   `~/.atlas/docs/user-guide/`. The available guides are:
   ...
   `~/.atlas/README.md` for the comprehensive reference.

3. To **modify config** for the user, edit `~/.atlas/config.toml` with search_replace.

4. To **create a skill** for the user, create `~/.atlas/skills/<name>/SKILL.md`
   (read `~/.atlas/docs/user-guide/08-skills.md` for the SKILL.md format).