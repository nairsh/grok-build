// claude_import.rs
// Scans Claude settings and generates TOML patches for .atlas/config.toml.
//
...
pub enum ImportScope {
    ...
    Global,
    /// Project-level: writes to `<repo>/.atlas/config.toml`.
    Project,
}
...
pub struct ImportPlan {
    ...
    /// Items to write to `<repo>/.atlas/config.toml`.
...
//
// The marker `[claude_compat] imported = true` in `~/.atlas/config.toml` is
...
///
/// Reads `[claude_compat] imported = true` from `~/.atlas/config.toml` once
...
/// overwritten).
///
/// Project items are written to `<repo_root>/.atlas/config.toml` (discovered
/// via `git2::Repository::discover`), not `cwd/.atlas/config.toml`, to avoid
...
pub fn apply_import(plan: &ImportPlan, cwd: &Path) -> anyhow::Result<ImportResult> {
    ...
    if !plan.project_items.is_empty() {
        let project_root = find_project_root(cwd);
        let project_path = project_root.join(".atlas").join("config.toml");
        ...
        let hooks_dir = project_root.join(".atlas").join("hooks");
...
fn apply_items_to_config(config_path: &Path, items: &[ImportableItem]) -> anyhow::Result<usize> {
    ...
    for item in items {
        match item {
            ...
            // Hooks are written to .atlas/hooks/ JSON files in apply_hooks_to_dir,
...
/// (Claude-compatible). The native hooks loader scans `.atlas/hooks/*.json`
...
fn apply_hooks_to_dir(hooks_dir: &Path, items: &[ImportableItem]) -> anyhow::Result<usize> {
    ...
    if count > 0 || dirty {
        ...
        info!(
            path = %target.display(),
            count,
            "Wrote imported hooks to .atlas/hooks/imported-from-claude.json"
        );
    }
...
"#;
    ...
    fn scan_claude_path_dirs_dedupes_global_and_project_when_same() {
        ...
        // requires `ATLAS_HOME` plumbing which is intentionally out of scope.
    ...
    fn gate_marker_cache_unset_means_uses_disk() {
        ...
        // dev's real `~/.atlas/config.toml` may legitimately have the marker
