...
/// Walks from cwd up to the git repo root checking `.atlas/config.toml` at each level.
/// Project-scoped `.atlas/config.toml` entries override global `~/.atlas/config.toml`
...
/// `.atlas/config.toml`, otherwise user (global config, `~/.claude.json`,
...
/// Load MCP servers with project-scoped overrides from `.atlas/config.toml`.
///
/// Merge strategy:
/// 1. Load MCP servers from global `~/.atlas/config.toml`
/// 2. Walk from git repo root down to `cwd`, loading `.atlas/config.toml` at each level
        ...
        if let Ok(root) = crate::config::load_config_file(config_path) {
            ...
            if !project_servers.is_empty() {
                tracing::info!(
                    ...
                    "Loaded project-scoped MCP servers from .atlas/config.toml"
                );
...
///
/// Uses the top-level `disabled_mcp_servers` array in `~/.atlas/config.toml`.
...
/// config file, e.g. a project-scoped `.atlas/config.toml`.
...
/// config file, e.g. a project-scoped `.atlas/config.toml`. OAuth credential
...
/// project-scoped `.atlas/config.toml` files.
...
/// (`"user"` or `"project"`).
///
/// Overlays project-scoped `.atlas/config.toml` files from `cwd` up to the
...
/// Path to a project-level config file (`<dir>/.atlas/config.toml`).
pub fn project_config_path(dir: &std::path::Path) -> PathBuf {
    dir.join(".atlas").join("config.toml")
}
