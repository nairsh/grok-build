...
/// Env lane (desktop `ATLAS_SESSION_LIST_CONVERSATIONS`) OR process-wide
/// `--chat` (`ATLAS_CHAT_MODE`); hard-off in release builds.
...
mod tests {
    ...
    fn conversations_lane_env_gating_matrix() {
        {
            let _off = xai_grok_test_support::EnvGuard::unset("ATLAS_SESSION_LIST_CONVERSATIONS");
            assert!(!conversations_lane_enabled());
        }
        {
            let _on = xai_grok_test_support::EnvGuard::set("ATLAS_SESSION_LIST_CONVERSATIONS", "1");
            assert_eq!(conversations_lane_enabled(), false);
        }
        {
            let _off = xai_grok_test_support::EnvGuard::set("ATLAS_SESSION_LIST_CONVERSATIONS", "0");
            assert!(!conversations_lane_enabled());
        }
    }
    ...
    fn conversations_lane_active_truth_table() {
        use crate::agent::chat_modes::ATLAS_CHAT_MODE_ENV;
        let _chat_off = xai_grok_test_support::EnvGuard::unset(ATLAS_CHAT_MODE_ENV);
        let _desktop_off =
            xai_grok_test_support::EnvGuard::unset("ATLAS_SESSION_LIST_CONVERSATIONS");
        assert!(
        ...
        {
            let _desktop =
                xai_grok_test_support::EnvGuard::set("ATLAS_SESSION_LIST_CONVERSATIONS", "1");
            assert_eq!(conversations_lane_active(), false);
        }
        {
            let _chat = xai_grok_test_support::EnvGuard::set(ATLAS_CHAT_MODE_ENV, "1");
            assert_eq!(
                conversations_lane_active(),
                false,
    ...
    fn parse_list_req_forces_kind_under_process_chat_mode_only() {
        ...
        .to_string();
        {
            let _off = xai_grok_test_support::EnvGuard::unset(ATLAS_CHAT_MODE_ENV);
        ...
        {
            let _on = xai_grok_test_support::EnvGuard::set(ATLAS_CHAT_MODE_ENV, "1");
            let req = parse_list_req(&raw).expect("parse");
            let parsed = ParsedMeta::parse(req.meta.as_ref());
            let expected = "build";
            assert_eq!(
