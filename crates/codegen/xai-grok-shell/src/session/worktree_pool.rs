...
//!   under `~/.atlas/worktree_pool/<instance_id>/` with a `.pid` liveness
...
/// `~/.atlas/worktree_pool/<instance_id>/` with a `.pid` file recording
...
impl WorktreePool {
    ...
    /// `~/.atlas/worktree_pool/<instance_id>/`. A `.pid` file is written so
...
pub struct AdoptableWorktree {
    /// Current path: `~/.atlas/worktree_pool/<old_instance>/<pool_id>/`
...
/// `~/.atlas/worktree_pool/`, reads each `.pid` file, and checks
...
/// `CleanAll` mode. Placed under `~/.atlas/worktree_pool/<instance_id>/<uuid>/`.
