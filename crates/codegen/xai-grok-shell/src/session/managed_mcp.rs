...
mod tests {
    ...
    fn lower_precedence_http_servers_are_blocked_by_toml_name_claims() {
        ...
        std::fs::create_dir_all(cwd.path().join(".atlas")).unwrap();
        std::fs::write(
            cwd.path().join(".atlas").join("config.toml"),
            r#"
[mcp_servers.github]
    ...
    fn load_plugin_mcp_creates_stdio_server_with_env_substitution() {
        let config: crate::util::config::McpConfig = serde_json::from_value(serde_json::json!({
            "mcpServers": {
                "echo-mcp": {
                    "command": "python3",
                    "args": ["${ATLAS_PLUGIN_ROOT}/mcp-echo-server.py"]
                }
            }
        }))
        ...
        let (servers, _) = load_plugin_mcp_servers_from_config(
            &config,
            "team-tool",
            "/home/user/.atlas/plugins/team-tool",
            "/home/user/.atlas/plugin-data/team-tool",
        );
        ...
        match &servers[0] {
            acp::McpServer::Stdio(acp::McpServerStdio {
                ...
                assert_eq!(
                    args.as_slice(),
                    &["/home/user/.atlas/plugins/team-tool/mcp-echo-server.py"]
                );
            }
