...
/// Override with `ATLAS_PREFIRE_LEAD_PERCENT`.
const DEFAULT_PREFIRE_LEAD_PERCENT: u64 = 10;
fn prefire_lead_percent() -> u64 {
    std::env::var("ATLAS_PREFIRE_LEAD_PERCENT")
        .ok()
...
mod two_pass_prefire_helper_tests {
    ...
    fn prefire_lead_percent_defaults_to_10() {
        unsafe { std::env::remove_var("ATLAS_PREFIRE_LEAD_PERCENT") };
...
impl SessionActor {
    ...
    async fn run_prefire_pass1_inner(self: &Arc<Self>) -> PrefirePass1Run {
        ...
        if std::env::var("ATLAS_DEBUG_TWO_PASS_FAIL_PASS1")
            .is_ok_and(|v| matches!(v.trim(), "1" | "true" | "yes" | "on"))
        {
            tracing::info!(
                target : "two_pass",
                "two_pass: DEBUG ATLAS_DEBUG_TWO_PASS_FAIL_PASS1 — prefire pass1 produces no cache"
            );
