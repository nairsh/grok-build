...
///
/// These directories under `~/.atlas/skills/` will be deleted on startup
...
fn resolve_skill_content(name: &str, raw: &str, grok_home: &std::path::Path) -> String {
    match name {
        ...
        "help" => {
            ...
            raw.replace("~/.atlas/", &grok_home_str)
        }
        _ => raw.to_string(),
    }
}

/// Extract bundled files to `~/.atlas/` on startup.
///
...
mod tests {
    ...
    async fn help_skill_discovered_by_skill_pipeline() {
        ...
        std::fs::create_dir_all(workspace.join(".atlas").join("skills").join("help")).unwrap();
        std::fs::copy(
            home.join("skills/help/SKILL.md"),
            workspace.join(".atlas/skills/help/SKILL.md"),
        )
        .unwrap();
