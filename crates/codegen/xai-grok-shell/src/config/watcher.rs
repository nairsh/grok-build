...
pub enum ConfigChangeEvent {
    AuthChanged,
    GlobalConfigChanged,
    /// `~/.atlas/models_cache.json` changed — the on-disk `/v1/models`
    /// catalog cache was rewritten, possibly by **another** grok process
    /// sharing the same `~/.atlas` (the writer may also be this process;
...
/// Watches `~/.atlas/` for `auth.json`, `config.toml`, and `models_cache.json`
/// changes, plus any extra paths (project `.atlas/config.toml`, `.mcp.json`,
...
/// `<cwd>/.atlas/` (catches `<cwd>/.atlas/config.toml`). Recursing on `<cwd>`
...
impl ConfigFileWatcher {
    ...
    /// `<cwd>/.atlas/`. Use [`Self::watch_path`] later to register additional
    ...
    pub fn start(
        ...
        // includes `<cwd>/.atlas/config.toml` so the loop above
        // watches `<cwd>/.atlas/`), the call below installs a
    ...
    /// Register `<cwd>/` and `<cwd>/.atlas/` as **non-recursive** watch
    ...
    /// `<cwd>/.mcp.json` and `<cwd>/.atlas/config.toml` trigger a
    ...
    /// `<cwd>/.atlas/`) previously registered for `cwd` via
...
///
/// **Known limitation:** if `<cwd>/.atlas/` does not yet
/// exist at session-open time, the `.atlas/` watch fails ENOENT and is
/// swallowed at `debug!`. A later `mkdir <cwd>/.atlas/` followed by a
/// write to `<cwd>/.atlas/config.toml` will NOT be observed — the
...
fn watch_cwd_dirs(debouncer: &mut Debouncer<AccessFilteredWatcher>, cwd: &Path) {
        ...
        .watch(&grok_dir, RecursiveMode::NonRecursive)
    {
        log_watch_error(
            &e,
            "failed to watch project .atlas directory (non-recursive)",
        );
    }
}
...
fn unwatch_cwd_dirs(debouncer: &mut Debouncer<AccessFilteredWatcher>, cwd: &Path) {
    ...
    if let Err(e) = debouncer.watcher().unwatch(&grok_dir) {
        tracing::debug!(error = %e, "failed to unwatch project .atlas directory");
    }
}

/// Log a `notify` watch failure, distinguishing the benign
/// "directory doesn't exist yet" case (logged at `debug!` — it's
/// expected for a freshly-opened session whose `<cwd>/.atlas/` hasn't
...
/// Watches skill directories (`~/.atlas/skills/`, `<repo>/.atlas/skills/`, etc.)
...
/// recursively: `grok_home` (`~/.atlas`, or `$ATLAS_HOME`) or a known vendor dir
/// directly under `$HOME` ([`HOME_VENDOR_DIRS`]).
///
/// These hold large non-skill trees — `~/.atlas` alone has `worktrees/`,
...
const HOME_VENDOR_DIRS: &[&str] = &[".atlas", ".agents", ".claude", ".cursor"];
...
/// `~/.atlas` these exist at startup; later creation is caught on restart).
...
mod tests {
    ...
    fn is_global_config_dir_matches_only_grok_home_and_vendor_dirs() {
        ...
        assert!(!g(&home.join("repo").join(".atlas")));
    }

    /// Regression for the ~/.atlas inotify-exhaustion / worktree-noise bug: a
    /// `SKILL.md` under a sibling subtree (e.g. `~/.atlas/worktrees/`) must not
    ...
    /// A write to `<cwd>/.atlas/config.toml` must surface as
    ...
    fn project_cwd_toml_triggers_reload() {
        ...
        let project_grok = cwd.path().join(".atlas");
        ...
        assert!(
            found,
            "expected ProjectConfigChanged for <cwd>/.atlas/config.toml within 2s"
        );
    }
    ...
    /// `start` must light up `<new_cwd>/.atlas/config.toml` writes
    ...
    fn watch_path_dynamic_registration() {
        ...
        let project_grok = new_cwd.path().join(".atlas");
        fs::create_dir_all(&project_grok).unwrap();
        fs::write(project_grok.join("config.toml"), "").unwrap();
