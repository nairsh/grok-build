...
pub enum ConfigUpdate {
    ...
    ///
    /// 1. The global `[mcp_servers]` table in `~/.atlas/config.toml`
    ///    changed.
    ...
    ///
    /// Project-scoped changes (`<cwd>/.atlas/config.toml`,
    ...
    /// (`<cwd>/.atlas/config.toml`, `<cwd>/.mcp.json`, or
    ...
    ProjectMcpServersChanged {
        /// The project root whose `.atlas/`, `.mcp.json`, or
    ...
    ModelsChanged,
    /// `~/.atlas/models_cache.json` was rewritten on disk (possibly by another
...
impl ConfigReloader {
    ...
    pub async fn run(
        ...
        loop {
            ...
            // same tick (e.g. `~/.atlas/config.toml` AND
    ...
    fn reload_config(&mut self) -> anyhow::Result<()> {
        ...
        // config (`~/.atlas/config.toml`) via toml::Value. Project-
        // scoped changes (`<cwd>/.atlas/config.toml`,
        ...
        let old_explore_model = self
            .last_global_config
            .get("ui")
            .and_then(|ui| ui.get("explore_model"));
        let new_explore_model = new_global.get("ui").and_then(|ui| ui.get("explore_model"));
        if old_model_table != new_model_table
            || old_models_table != new_models_table
            || old_task_model != new_task_model
            || old_explore_model != new_explore_model
        {
...
/// |----------------------------|-------------------------|-------------------|
/// | `ProjectConfigChanged`     | `<cwd>/.atlas/config.toml` | `<cwd>`           |
...
/// for `.atlas/config.toml`, `find_mcp_json_files` for `.mcp.json`) so
...
/// (`~/.atlas/config.toml`, `~/.claude.json`, `~/.cursor/mcp.json`)
...
mod tests {
    ...
    /// The hash must reflect ancestor `.atlas/config.toml` and `.mcp.json`
    ...
    fn hash_project_mcp_config_covers_ancestors() {
        ...
        std::fs::write(tmp.path().join(".atlas").join("config.toml"), "x = 1").unwrap();
        let h3 = hash_project_mcp_config(&child).expect("readable");
        assert_ne!(
            h2, h3,
            "ancestor .atlas/config.toml create must change the hash"
        );
    }
    ...
    fn parse_skills_config_with_paths() {
        let config: toml::Value = toml::from_str(
            r#"
[skills]
paths = ["/home/user/.atlas/skills"]
ignore = ["/tmp"]
"#,
        )
        .unwrap();
        let skills = parse_skills_config(&config);
        assert_eq!(skills.paths, vec!["/home/user/.atlas/skills".to_string()]);
...
"#,
    ...
    /// `ProjectConfigChanged` (`<cwd>/.atlas/config.toml`) and
    ...
    fn collect_project_cwds_excludes_home_claude_json() {
        let batch = vec![
            ...
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/x/.atlas/config.toml"),
            },
        ];
    ...
    fn collect_project_cwds_dedupes_and_extracts() {
        let batch = vec![
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/a/.atlas/config.toml"),
            },
            ConfigChangeEvent::McpConfigChanged {
                path: PathBuf::from("/repo/a/.mcp.json"),
            },
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/b/.atlas/config.toml"),
            },
        ];
        let cwds = collect_project_cwds(&batch);
        assert_eq!(
