...
//! 4. Smoke-validate session JSON under `$ATLAS_HOME/sessions/` after exit.
//!
//! Requires a built `grok` binary (`ATLAS_BINARY` or cargo-built pager) for the
...
fn trusted_local_refresh_surfaces_new_agent_via_discovery() {
    ...
    let _grok_guard = EnvVarGuard::set("ATLAS_HOME", &grok_home);
    ...
    // grok_home() caches the first ATLAS_HOME per process; a separate test
...
async fn headless_session_refreshes_trusted_local_plugin_and_writes_session_json() {
    ...
    // The spawned binary gets HOME/ATLAS_HOME via `cmd.env` below; this global env
    ...
    // Smoke: session storage under ATLAS_HOME/sessions has JSON artifacts.
