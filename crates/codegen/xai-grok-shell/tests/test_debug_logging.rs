...
//! caller-owned `$ATLAS_HOME`, then inspects `~/.atlas/debug/`:
...
//!   `~/.atlas/debug/<sessionId>.txt` with first-party content, and does NOT enable
//!   sampling/instrumentation. Regression for the master switch having bundled
//!   `ATLAS_LOG_SAMPLING`/`ATLAS_INSTRUMENTATION`, whose global `TargetFilterLayer`
...
//! - a live `agent` session (explicit `ATLAS_DEBUG_LOG=1`) writes a per-session
//!   `~/.atlas/debug/<sessionId>.txt` with real first-party content + `latest.txt`.
//! - `--debug-file <path>` writes one explicit file and bypasses per-session
//!   routing entirely (no `~/.atlas/debug/` files).
//! - `ATLAS_LOG_FILE=<path>` writes that explicit file (back-compat single file).
//!
...
/// The per-session firehose directory under a pinned `$ATLAS_HOME`.
fn debug_dir(home: &Path) -> PathBuf {
    home.join(".atlas").join("debug")
}

/// List firehose `*.txt` files under `~/.atlas/debug` (excluding the `latest.txt`
...
/// Build a headless `grok -p` command with a pinned `$ATLAS_HOME` so the firehose
/// lands under `<home>/.atlas/debug`. Firehose env knobs are cleared so the test
...
fn debug_cmd(
    ...
    cmd.env("ATLAS_HOME", home.join(".atlas"));
    cmd.env_remove("ATLAS_DEBUG_LOG");
    cmd.env_remove("ATLAS_LOG_FILE");
    cmd.env_remove("ATLAS_LOG_SAMPLING");
    cmd.env_remove("ATLAS_HOOKS_LOG");
    cmd
}
...
/// A live `agent` session writes `~/.atlas/debug/<sessionId>.txt` with real
...
async fn agent_session_writes_named_session_file() {
    with_local_set(|| async {
        ...
        let client = GrokStdioClient::spawn_with_home_and_env(
            ...
            workdir.path(),
            home,
            &[("ATLAS_DEBUG_LOG", "1"), ("ATLAS_HOME", &grok_home_str)],
        )
        .await;
        ...
        // file, so `tail -f ~/.atlas/debug/latest.txt` follows the live session.
        #[cfg(unix)]
        {
...
/// The `--debug` FLAG (not `ATLAS_DEBUG_LOG` directly) drives the firehose end to
/// end through the master switch. Regression: the master switch used to also set
/// `ATLAS_LOG_SAMPLING`/`ATLAS_INSTRUMENTATION`, whose `TargetFilterLayer` globally
...
async fn debug_flag_master_switch_enables_firehose() {
    with_local_set(|| async {
        ...
        // clears ATLAS_LOG_SAMPLING (hermetic), so the sampling layer stays off and
        // `~/.atlas/logs/sampling.jsonl` is never written — the `--debug`
        ...
        // ATLAS_INSTRUMENTATION=disabled, so that assertion would be vacuous.
...
/// `--debug-file <path>` writes one explicit file and bypasses per-session
/// routing entirely (no `~/.atlas/debug/` files created).
#[tokio::test]
#[ignore] // requires pre-built binary; run with --ignored
...
/// `ATLAS_LOG_FILE=<path>` (no `--debug`) writes that exact file (back-compat).
#[tokio::test]
#[ignore] // requires pre-built binary; run with --ignored
async fn grok_log_file_explicit_path_is_written() {
    ...
    cmd.env("ATLAS_LOG_FILE", &custom);
    let result = run_headless_with_cmd(cmd).await;

    assert_headless_success(&result, "grok ATLAS_LOG_FILE=path", Some(&server));
    assert_no_crashes(&result.stderr);
    assert!(
        custom.exists(),
        "explicit ATLAS_LOG_FILE path not written: {custom:?}\nstderr tail:\n{}",
        stderr_tail(&result.stderr, 800)
    );
    // Single-file mode bypasses per-session routing.
    assert!(
        firehose_txt_files(home.path()).is_empty(),
        "ATLAS_LOG_FILE must bypass per-session routing, found: {:?}",
        firehose_txt_files(home.path())
    );
}
