...
fn invalidate_models_cache(home: &std::path::Path) {
    let cache = home.join(".atlas").join("models_cache.json");
    if cache.exists() {
...
/// The `ATLAS_AGENT` escape hatch should override the model's agent_type.
/// Setting `ATLAS_AGENT=grok-build` with an alternate-agent model should use
/// grok-build harness.
#[tokio::test]
#[ignore]
async fn test_grok_agent_env_overrides_model_agent_type() {
    with_local_set(|| async {
            ...
            cmd.env("ATLAS_AGENT", "grok-build");
            ...
            assert!(
                sys_prompt.contains("Grok") || sys_prompt.contains("grok"),
                "ATLAS_AGENT=grok-build should override cursor model's agent_type\nsystem prompt preview: {}",
                & sys_prompt[..sys_prompt.len().min(500)]
            );
        })
        .await;
