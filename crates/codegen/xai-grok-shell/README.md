...
Credentials are stored in `~/.atlas/auth.json` and persist across sessions. Tokens expire after 7 days; Grok will prompt you to re-authenticate when needed.
...
export ATLAS_OIDC_ISSUER="https://acme.okta.com"
export ATLAS_OIDC_CLIENT_ID="0oa1b2c3d4e5f6g7h8i9"
```

Customers typically also override the API endpoint to point at their own proxy:
```bash
export ATLAS_CLI_CHAT_PROXY_BASE_URL="https://grok-proxy.acme.com/v1"
```

**3. Run `grok`.** The CLI discovers endpoints via `{issuer}/.well-known/openid-configuration`, opens the IdP login page, and stores tokens in `~/.atlas/auth.json`. The OIDC token is sent as `Authorization: Bearer` to the configured proxy. Tokens auto-refresh silently via the stored `refresh_token`.
...
4. **stdout** → captured by Grok and saved to `~/.atlas/auth.json` as the access token
...
```bash
# Or via environment variables
export ATLAS_AUTH_PROVIDER_COMMAND="/usr/local/bin/my-auth-provider"
export ATLAS_AUTH_PROVIDER_LABEL="Acme Corp"   # optional
export ATLAS_AUTH_TOKEN_TTL=3600               # optional
```
...
When Grok needs to refresh an expired token, it re-runs your binary with `ATLAS_AUTH_EXPIRED=1` set in the environment. Your binary can use this to take a faster silent-refresh path:

```bash
#!/bin/sh
if [ "$ATLAS_AUTH_EXPIRED" = "1" ]; then
...
`ATLAS_AUTH_EXPIRED` is optional — if your binary ignores it, Grok still works. It just runs the same flow for both login and refresh.
...
export ATLAS_AUTH_EARLY_INVALIDATION_SECS=300
```
...
curl -s -N -X POST "https://cli-chat-proxy.grok.com/v1/chat/completions" \
  ...
  -H "Authorization: Bearer $(jq -r '."https://accounts.x.ai/sign-in".key' ~/.atlas/auth.json)" \
...
| -------------------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Authorization: Bearer <token>`  | Yes      | Session token from `~/.atlas/auth.json` (set by `grok login`)                                                                                                                              |
...
Grok reads configuration from `~/.atlas/config.toml`. If the file doesn't exist, Grok uses sensible defaults. You only need to specify values you want to override.
...
telemetry = false                      # anonymous usage telemetry (env: ATLAS_TELEMETRY_ENABLED)
feedback = false                       # feedback system (env: ATLAS_FEEDBACK_ENABLED)
...
[tools]
respect_gitignore = true               # filter gitignored files from tools (env: ATLAS_RESPECT_GITIGNORE)
...
```toml
[telemetry]
events_url = "https://example.com/events"  # env: ATLAS_TELEMETRY_EVENTS_URL
events_api_key = "..."                      # env: ATLAS_TELEMETRY_EVENTS_API_KEY
mixpanel_token = "..."                      # env: ATLAS_TELEMETRY_MIXPANEL_TOKEN
mixpanel_enabled = true                     # env: ATLAS_TELEMETRY_MIXPANEL_ENABLED
trace_upload = true                         # env: ATLAS_TELEMETRY_TRACE_UPLOAD
```

When building from source, defaults can also be baked into the binary at compile time by setting `ATLAS_TELEMETRY_BUILD_EVENTS_URL`, `ATLAS_TELEMETRY_BUILD_EVENTS_API_KEY`, and `ATLAS_TELEMETRY_BUILD_MIXPANEL_TOKEN` in the build environment (providing a Mixpanel token this way also enables Mixpanel by default). Config-file and runtime env values override build-time defaults.
...
- project config: `<repo>/.atlas/lsp.json`
- user config: `~/.atlas/lsp.json`
...
- LSP tools are enabled (`ATLAS_LSP_TOOLS=1` or `[features] lsp_tools = true`)
- the merged LSP configuration is non-empty

Enable the tool for one run:

```bash
ATLAS_LSP_TOOLS=1 grok
```
...
- Passive diagnostics do **not** require `ATLAS_LSP_TOOLS=1`; they run whenever an applicable server is configured and starts successfully.
...
| --------------------------- | ----- | -------- |
| `./.atlas/skills/`           | Local | Highest  |
| `<repo_root>/.atlas/skills/` | Repo  | Medium   |
| `~/.atlas/skills/`           | User  | Lowest   |
| `~/.claude/skills/`         | User  | Lowest   |

Skills with the same name are deduplicated — higher priority locations override lower ones.

Repo-scoped skills (Local and Repo) respect `.gitignore` and are filtered out if ignored. User-scoped skills (`~/.atlas/skills/`) are outside the repo and never filtered.
...
Grok discovers agent definitions from `.atlas/agents/` (project), `~/.atlas/agents/` (user), and built-in agents. Priority (highest wins):

1. `--agent-profile <PATH>` CLI flag
2. `[agent]` section in `config.toml`
3. `ATLAS_AGENT` env var
...
# or
export ATLAS_AGENT="my-custom-agent"
```
...
```bash
export ATLAS_SUBAGENTS=0              # Environment variable
```
...
# instructions_file = ".atlas/personas/concise.md"  # or load from file
```

Both are also discovered from `.atlas/roles/*.toml` and `.atlas/personas/*.toml` files respectively. If a requested persona is not found, the spawn fails (fail-closed).
...
Grok discovers hooks from `.atlas/hooks/` in the project directory. Manage them with:
...
**Web search model:** Set `[models] web_search`, `ATLAS_WEB_SEARCH_MODEL`, or `--web-search-model` to point the `web_search` tool at a different model. The target endpoint must support the Responses API and web search.
...
|----------|----------|-------------|
| `ATLAS_MODELS_BASE_URL` | Yes | Base URL for inference / chat completions (e.g. `https://api.acme.com/v1`). The model list is fetched from `{base_url}/models` automatically |
| `XAI_API_KEY` | Yes | API key sent as `Authorization: Bearer` to the custom endpoint |
| `ATLAS_MODELS_LIST_URL` | No | Override the model list URL if it differs from `{base_url}/models` |
...
Grok fetches the model list from `{ATLAS_MODELS_BASE_URL}/models` on startup and sends inference requests to `ATLAS_MODELS_BASE_URL`. This follows the standard OpenAI-compatible convention used by OpenAI, Anthropic, OpenRouter, Groq, Together.ai, and others.

If your model list endpoint differs from `{base_url}/models`, set `ATLAS_MODELS_LIST_URL` explicitly.

**Combining with `[endpoints]` config:** You can also set endpoints in `~/.atlas/config.toml`:
...
MCP servers are configured in `~/.atlas/config.toml`:
...
MCP servers can also be configured per-project in `.atlas/config.toml`. Grok walks from the current directory up to the git repo root, loading `.atlas/config.toml` at each level:

| Location                        | Scope             | Priority |
| ------------------------------- | ----------------- | -------- |
| `~/.atlas/config.toml`           | All projects      | Lowest   |
| `<repo-root>/.atlas/config.toml` | This repository   | ↑        |
...
**Example:** commit a `.atlas/config.toml` in your repo to share MCP servers across the team:
...
If you also have a `linear` server in `~/.atlas/config.toml`, the project version replaces it entirely.

> **Note:** Only `[mcp_servers]` is supported in project-scoped `.atlas/config.toml`. Other config sections (models, shortcuts, etc.) are only read from `~/.atlas/config.toml`.
...
> **Experimental:** requires `--experimental-memory` (or `ATLAS_MEMORY=1` / `[memory] enabled = true` in config).
...
- **Global** (`~/.atlas/memory/MEMORY.md`) — facts that apply across all your projects
- **Workspace** (`~/.atlas/memory/<project-slug>-<hash8>/MEMORY.md`) — project-specific conventions and context
- **Session logs** (`~/.atlas/memory/<project-slug>-<hash8>/sessions/`) — per-session summaries
...
For richer capture — decisions, patterns, debugging workflows, API discoveries — use `/flush` in the TUI. This triggers an LLM-generated summary of the current session's most important content and writes it to a dated session log under `~/.atlas/memory/<project-slug>-<hash8>/sessions/`, where it is indexed and searchable in future sessions.
...
Key options under `[memory]` in `~/.atlas/config.toml`:
...
| `watcher.enabled` | `true` | Watch `~/.atlas/memory/` for external edits and reindex on search |
...
| `workspace`     | Everywhere         | CWD + `/tmp` + `~/.atlas/` | Allowed       | Normal development       |
| `read-only`     | Everywhere         | `~/.atlas/` only           | Blocked       | Exploration, code review |
| `strict`        | CWD + system paths | CWD + `/tmp` + `~/.atlas/` | Blocked       | Untrusted code           |

Sensitive paths (`~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.atlas/auth/`) are always
write-protected regardless of profile.

### Custom Profiles

Create `~/.atlas/sandbox.toml` (global) or `.atlas/sandbox.toml` (per-project):
...
Sandbox events (profile applied, violations) are logged to `~/.atlas/sandbox-events.jsonl`
...
- **Skills** — from `.atlas/skills/`, `~/.atlas/skills/`, plugins, and config paths
...
Grok automatically discovers configuration from Claude Code directories alongside native `.atlas/` paths. No extra setup is needed.

### What is picked up

| Component         | Claude Code location                                 | How Grok uses it                 |
| ----------------- | ---------------------------------------------------- | -------------------------------- |
| **Skills**        | `.claude/skills/`, `~/.claude/skills/`               | Loaded as skills (same as `.atlas/skills/`) |
...
Fetch a specific URL and return its content as markdown. **Disabled by default** — enable with `ATLAS_WEB_FETCH=1`. 
...
Sessions are stored under `~/.atlas/sessions/`, organized by URL-encoded working directory:
...
| Path                  | Description                                         |
| --------------------- | --------------------------------------------------- |
| `~/.atlas/config.toml` | Configuration file                                  |
...
| `.atlas/config.toml`   | Project-scoped config (MCP servers)                 |
...
| `ATLAS_CLI_CHAT_PROXY_BASE_URL`  | Override the cli-chat-proxy URL (default: `https://cli-chat-proxy.grok.com/v1`)                          |
| `ATLAS_MODELS_BASE_URL`          | Custom base URL for inference. Model list auto-fetched from `{base_url}/models` (see [Custom Models Endpoint](#custom-models-endpoint)) |
| `ATLAS_MODELS_LIST_URL`          | Override the model list URL if it differs from `{ATLAS_MODELS_BASE_URL}/models`                                              |
| `ATLAS_AUTH_PROVIDER_COMMAND`     | External auth binary (alternative to config file). See [External Auth Provider](#external-auth-provider) |
| `ATLAS_AUTH_TOKEN_TTL`            | Token lifetime in seconds for external auth providers that output bare tokens. See [External Auth Provider](#external-auth-provider) |
| `ATLAS_AUTH_EARLY_INVALIDATION_SECS` | Seconds before `expires_at` to consider a token expired (default: `300`). See [Automatic Credential Refresh](#automatic-credential-refresh) |
| `ATLAS_OIDC_ISSUER`              | OIDC issuer URL (alternative to config file). See [OIDC](#oidc-customer-sso)                             |
| `ATLAS_OIDC_CLIENT_ID`           | OIDC client ID (alternative to config file). See [OIDC](#oidc-customer-sso)                              |
| `ATLAS_HOME`                     | Override config directory (default: `~/.atlas`)                                                           |
| `ATLAS_SUBAGENTS`                | Enable (`1`) or disable (`0`) subagent/task tool support                                                 |
| `ATLAS_MEMORY`                   | Enable (`1`) or disable (`0`) cross-session memory                                                       |
| `ATLAS_AGENT`                    | Custom agent definition path or name (see [Agent Profiles](#agent-profiles))                             |
| `ATLAS_WEB_FETCH`                | Enable (`1`) or disable (`0`) the `web_fetch` tool                                                       |
| `ATLAS_WEB_FETCH_PROXY`          | Egress proxy URL for `web_fetch` requests (overridden by `[toolset.web_fetch] proxy_endpoint`)           |
| `ATLAS_RESPECT_GITIGNORE`        | Disable `.gitignore` filtering in tools when set to `0`                                                  |
| `ATLAS_FEEDBACK_ENABLED`         | Enable (`1`) or disable (`0`) feedback system independently from telemetry                               |
| `ATLAS_DEPLOYMENT_KEY`           | Management API key for enterprise deployments                                                            |
| `ATLAS_LOG_FILE`                 | Enable file logging by providing a file path (the value is used verbatim as the path)                    |
| `ATLAS_DEBUG_LOG`                | Debug firehose (set by `--debug`): truthy routes per-session logs to `~/.atlas/debug/<sessionId>.txt`, a path writes that one file |
| `RUST_LOG`                      | Log filter for stderr (headless `-p` defaults to `off`, other non-TUI modes to `error`; TUI captures stderr) and for the `ATLAS_LOG_FILE` log; the `--debug` firehose ignores it |
...
```bash
mkdir -p ~/.atlas/completions/bash
grok completions bash > ~/.atlas/completions/bash/grok.bash
```

Add to `~/.bashrc`:

```bash
[[ -r "$HOME/.atlas/completions/bash/grok.bash" ]] && source "$HOME/.atlas/completions/bash/grok.bash"
```
...
```bash
mkdir -p ~/.atlas/completions/zsh
grok completions zsh > ~/.atlas/completions/zsh/_grok
```

Add to `~/.zshrc`:

```zsh
fpath=("$HOME/.atlas/completions/zsh" $fpath)
autoload -Uz compinit
compinit
```
...
Write logs to a file for debugging. The TUI captures stderr, so `RUST_LOG` alone won't produce visible output in production — use `grok --debug` or `ATLAS_LOG_FILE` instead:

```bash
# Per-session debug log (~/.atlas/debug/<sessionId>.txt)
grok --debug

# Log to a custom path
ATLAS_LOG_FILE=/tmp/grok-debug.log grok

# Tail the most-recently-opened session's log in another terminal (Unix symlink)
tail -f ~/.atlas/debug/latest.txt
```

The `--debug` firehose uses a fixed filter (first-party crates at `debug`) and is not narrowed by `RUST_LOG`. A `ATLAS_LOG_FILE` log defaults to `debug` and honors `RUST_LOG`, so you can set module-level filters for targeted debugging:

```bash
# Debug auth, info for everything else
ATLAS_LOG_FILE=/tmp/grok-debug.log RUST_LOG="info,xai_grok_shell::auth=debug" grok
```
...
cat ~/.atlas/sessions/<encoded-cwd>/<session-id>/summary.json | jq .

# View conversation history
cat ~/.atlas/sessions/<encoded-cwd>/<session-id>/updates.jsonl | head -20

# Count turns in a session
wc -l ~/.atlas/sessions/<encoded-cwd>/<session-id>/chat_history.jsonl
```
