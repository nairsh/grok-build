...
//! startup latency, under either strategy (`ATLAS_FSNOTIFY_PER_DIR=0|1`).
//!
//! ```bash
...
//! ATLAS_FSNOTIFY_PER_DIR=0 cargo run --release -p xai-fsnotify --example watch_stats -- run /tmp/js-repo 5
//! ATLAS_FSNOTIFY_PER_DIR=1 cargo run --release -p xai-fsnotify --example watch_stats -- run /tmp/js-repo 5
//! ```
//!
...
fn main() {
    ...
    match args.get(1).map(String::as_str) {
        ...
        Some("run") => {
            ...
            let strategy = std::env::var("ATLAS_FSNOTIFY_PER_DIR").unwrap_or_default();
