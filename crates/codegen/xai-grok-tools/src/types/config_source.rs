...
pub enum ConfigSource {
    ...
    Builtin,
    /// Bundled skill shipped with the binary (extracted to ~/.atlas/skills/
    /// or injected via bundled skill dirs).
    Bundled { path: PathBuf },
    /// Server-synced (e.g. ~/.atlas/server-skills from the skill store).
    Server { path: PathBuf },
    /// Project-scoped: cwd/.atlas/ or cwd/.claude/.
    Project { path: PathBuf },
    /// User-scoped: ~/.atlas/ or ~/.claude/.
    User { path: PathBuf },
