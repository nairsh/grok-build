//! LSP server configuration from `.atlas/lsp.json`.
...
pub fn load_servers_with_plugins_sourced(
    ...
    let project_path = cwd.join(".atlas").join("lsp.json");
...
/// Load LSP server configs from `~/.atlas/lsp.json` and `<cwd>/.atlas/lsp.json`.
