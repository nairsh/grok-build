...
/// handle `~/.atlas/worktrees/<repo>/<session>/`) and calls `remove_worktree()`
...
pub mod gc {
    ...
    /// symlinked `$ATLAS_HOME` or custom worktree path would otherwise never match.
...
mod tests {
    ...
    fn test_cleanup_worktrees_in_with_nested_dirs() {
        ...
        // Create ~/.atlas/worktrees/<repo>/<session>/ structure.
        let worktrees_dir = tmp.path().join("worktrees");
    ...
    fn test_cleanup_worktrees_in_removes_nested_dangling_symlink() {
        // Dangling symlink one level deeper (~/.atlas/worktrees/<repo>/<session>):
    ...
    fn remove_with_delegate_deregisters_plain_worktree_without_calling_delegate() {
        xai_test_utils::require_git!();
        use xai_test_utils::git::{git_commit_all, init_git_repo};
        // Isolate ATLAS_HOME so the post-removal unregister writes to a private DB.
        #[cfg(feature = "metadata")]
    ...
    fn delete_fallback_invokes_delegate_when_direct_delete_fails() {
        ...
        let report = delete_snapshot_with_delegate_fallback(
            ...
            Path::new("/home/u/.atlas/worktrees/repo/wt"),
            Some(&delegate),
    ...
    fn test_cleanup_orphaned_btrfs_skips_when_mount_target_parent_missing() {
        ...
        let mount_target = unrestored_home.join(".atlas/worktrees/x/wt-live");
        assert!(
    ...
    fn test_cleanup_orphaned_btrfs_skips_active() {
        ...
        let mount_target = std::path::PathBuf::from("/home/user/.atlas/worktrees/active-wt");
    ...
    mod metadata_integration {
        ...
        fn register_worktree_writes_correct_fields() {
            // Isolate ATLAS_HOME so register_worktree's open_default write lands
            ...
            // (ATLAS_HOME is process-global) can't INSERT-OR-REPLACE our row.
        ...
        fn gc_with_delegate_removes_expired_and_unregisters() {
            ...
            // ATLAS_HOME == the gc DB dir so remove_worktree's open_default
